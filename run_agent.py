...
_INSTANT_GREETING_RE = re.compile(
    r"^(?:"
    r"hi|hello|hey|yo|"
    r"привет|здравствуй|здравствуйте|доброе\s+утро|добрый\s+день|добрый\s+вечер|хай"
    r")[!\.\,\s]*$",
    re.IGNORECASE,
)


def _instant_first_turn_greeting(user_message: Any) -> Optional[str]:
    """Return a local greeting for trivial first-turn hellos."""
    if not isinstance(user_message, str):
        return None
    text = user_message.strip()
    if not text or len(text) > 40 or "\n" in text:
        return None
    if not _INSTANT_GREETING_RE.match(text):
        return None
    if re.search(r"[а-яё]", text, re.IGNORECASE):
        return "Привет! Я NOTPUNKS Agent. Чем могу помочь?"
    return "Hello! I am NOTPUNKS Agent. How can I help?"
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"NOTPUNKSAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    def __init__(
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        ...
                        "X-OpenRouter-Title": "NOTPUNKS Agent",
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `notpunks model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `notpunks model` to "
                        "select a provider, or run `notpunks setup` for first-time "
                        "configuration."
                    )
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by NOTPUNKS Agent.  Choose a model with at least "
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``notpunks chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `notpunks setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by NOTPUNKS "
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Pointer to the hermes-agent skill + docs for user questions about NOTPUNKS itself.
        ...
        # Wallet / NFT context injection (layer 4.5)
        try:
            from agent.wallet.context import build_wallet_context, format_context_for_prompt
            wallet_ctx = build_wallet_context(self.config)
            if wallet_ctx and wallet_ctx.verified:
                prompt_parts.append(format_context_for_prompt(wallet_ctx))
        except Exception:
            pass  # Wallet context is non-critical
    ...
    def run_conversation(
        ...
        # ``notpunks logs --session <id>`` can filter a single conversation.
        ...
        if not conversation_history:
            instant_greeting = _instant_first_turn_greeting(original_user_message)
            if instant_greeting:
                messages.append({"role": "assistant", "content": instant_greeting})
                self._session_messages = messages
                self._save_trajectory(messages, _summarize_user_message_for_log(user_message), True)
                self._cleanup_task_resources(effective_task_id)
                self._persist_session(messages, conversation_history)
                logger.info(
                    "Turn ended: reason=instant_first_turn_greeting model=%s "
                    "api_calls=0/%d budget=0/%d tool_turns=0 last_msg_role=assistant "
                    "response_len=%d session=%s",
                    self.model,
                    self.max_iterations,
                    self.iteration_budget.max_total if self.iteration_budget else 0,
                    len(instant_greeting),
                    self.session_id or "none",
                )
                self.clear_interrupt()
                self._stream_callback = None
                return {
                    "final_response": instant_greeting,
                    "last_reasoning": None,
                    "messages": messages,
                    "api_calls": 0,
                    "completed": True,
        ...
        # are reused.  The system prompt is NOTPUNKS's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for NOTPUNKS internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            # Thinking spinner / animation during API call
            thinking_spinner = None
            face = random.choice(KawaiiSpinner.get_thinking_faces())
            verb = random.choice(KawaiiSpinner.get_thinking_verbs())

            if self.thinking_callback:
                # CLI TUI mode: use prompt_toolkit widget (works in all modes)
                self.thinking_callback(f"{face} {verb}...")
            elif not self._has_stream_consumers() and self._should_start_quiet_spinner():
                # Raw KawaiiSpinner when no streaming consumers and safe sink
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    # support it.
                    def _stop_spinner():
                        """Stop raw stdout spinner on first delta; TUI spinner stays until response completes."""
                        nonlocal thinking_spinner
                        if thinking_spinner:
                            thinking_spinner.stop("")
                            thinking_spinner = None
                        # Do NOT clear thinking_callback here — the TUI thinking
                        # spinner should remain visible until the full response is
                        # received so users see animation during the entire wait.
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: notpunks login --provider nous")
                    ...
                    if (
                        ...
                        from hermes_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for NOTPUNKS-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: notpunks config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: notpunks config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # providers (DeepSeek, Kimi, MiMo, NOTPUNKS).  A 429 can
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `notpunks auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: notpunks setup", force=True)
