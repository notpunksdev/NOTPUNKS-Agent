...
_update_notice_sessions: set[str] = set()
...
def _format_update_notice(behind: int, command: str) -> str:
    commits = "commit" if behind == 1 else "commits"
    return (
        f"Update available: {behind} {commits} behind. Run {command}. "
        f"Доступно обновление: отставание на {behind}. Выполните {command}."
    )


def _emit_update_notice_once(sid: str, info: dict) -> None:
    try:
        behind = int(info.get("update_behind") or 0)
    except Exception:
        behind = 0
    if behind <= 0:
        return
    ...
    with _update_notice_lock:
        if sid in _update_notice_sessions:
            return
        _update_notice_sessions.add(sid)

    command = str(info.get("update_command") or "notpunks update")
    _emit(
        "status.update",
        sid,
        {"kind": "warn", "text": _format_update_notice(behind, command)},
    )


def _schedule_update_notice(sid: str, agent, *, emit_initial: bool = True) -> None:
    """Surface update notices even when the startup check finishes after init."""
    info = _session_info(agent)
    if emit_initial:
        _emit("session.info", sid, info)
    if info.get("update_behind") is not None:
        _emit_update_notice_once(sid, info)
        return

    def _wait_and_emit() -> None:
        try:
            from hermes_cli.banner import get_update_result
            from hermes_cli.config import recommended_update_command

            behind = get_update_result(timeout=15.0)
            if behind is None:
                return
            refreshed = _session_info(agent)
            refreshed["update_behind"] = behind
            refreshed["update_command"] = recommended_update_command()
            _emit("session.info", sid, refreshed)
            _emit_update_notice_once(sid, refreshed)
        except Exception:
            pass

    threading.Thread(
        target=_wait_and_emit,
        name=f"tui-update-notice-{sid}",
        daemon=True,
    ).start()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    system_prompt = append_language_instruction(system_prompt, get_config_language(cfg))
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    _schedule_update_notice(sid, agent)
...
def _(rid, params: dict) -> dict:
    ...
    def _build() -> None:
        ...
        try:
            ...
            _emit("session.info", sid, info)
            _emit_update_notice_once(sid, info)
            if info.get("update_behind") is None:
                _schedule_update_notice(sid, agent, emit_initial=False)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "language":
        ...
        raw = str(value or "").strip().lower()
        if raw in ("", "status"):
            current = normalize_language((_load_cfg().get("agent") or {}).get("language", "en"))
            return _ok(rid, {"key": key, "value": current, "label": language_label(current)})
        if raw not in {"en", "ru", "english", "russian", "eng", "rus", "английский", "русский"}:
            return _err(rid, 4002, f"unknown language: {value}")
        nv = normalize_language(raw)
        _write_config_key("agent.language", nv)
        resp = {"key": key, "value": nv, "label": language_label(nv)}
        if session and session.get("agent"):
            if session.get("running"):
                return _err(rid, 4009, "session busy — /interrupt the current turn before /language")
            history_reset, info = True, _reset_session_agent(params.get("session_id", ""), session)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "language":
        ...
        current = normalize_language((_load_cfg().get("agent") or {}).get("language", "en"))
        return _ok(rid, {"value": current, "label": language_label(current)})
    if key == "details_mode":
        allowed_dm = frozenset({"hidden", "collapsed", "expanded"})
        raw = (
            str(
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `hermes` is interactive — use `/notpunks chat -q …` or run `hermes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`notpunks setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`notpunks gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`notpunks sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`notpunks config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `notpunks model` and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "command":
            ...
            if not command:
                command = f"/skills {query}".strip()
            if not command.startswith("/skills"):
                command = f"/skills {command}".strip()
            handle_skills_slash(command, console=console)
            return _ok(rid, {"output": sink.getvalue()})
        if action == "list":
            from hermes_cli.banner import get_available_skills
