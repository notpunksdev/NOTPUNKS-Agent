...
def test_config_set_language_resets_live_session(monkeypatch):
    session = _session(
        agent=types.SimpleNamespace(),
    ...
    resp = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"session_id": "sid", "key": "language", "value": "ru"},
        }
    )

    assert resp["result"]["value"] == "ru"
    assert resp["result"]["label"] == "Русский"
    assert resp["result"]["history_reset"] is True
    assert resp["result"]["info"] == {"model": "x"}
    assert session["history"] == []
