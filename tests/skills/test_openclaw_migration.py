...
"""
    ...
    assert "Tyler Williams > Active Projects: NOTPUNKS Agent" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``NOTPUNKS``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "NOTPUNKS prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told NOTPUNKS to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "NOTPUNKS config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "NOTPUNKS uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (NOTPUNKS home) when rebranding
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "NOTPUNKS remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "hermes prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "NOTPUNKS was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "NOTPUNKS said to ask NOTPUNKS about NOTPUNKS settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.hermes`` (the real NOTPUNKS home), not the broken
    ``.Hermes``.
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are NOTPUNKS" in result
