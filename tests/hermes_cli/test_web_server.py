...
class TestNewEndpoints:
    ...
    def test_marketplace_skill_status_public(self, monkeypatch):
        from starlette.testclient import TestClient
        ...
        resp = TestClient(app).get("/api/skills/marketplace/status?name=alpha")
        assert resp.status_code == 200
        assert resp.json() == {
            "ok": True,
            "skills": [{
    ...
    def test_marketplace_skill_status_allows_skills_origin(self, monkeypatch):
        from starlette.testclient import TestClient
        import hermes_cli.skill_marketplace as skill_marketplace
        from hermes_cli.web_server import app
        ...
        resp = TestClient(app).get(
            ...
            headers={"Origin": "https://skilzzz.com"},
        )

        assert resp.status_code == 200
        assert resp.headers["access-control-allow-origin"] == "https://skilzzz.com"

    def test_marketplace_creator_endpoints_public_for_skills_origin(self, monkeypatch):
        from starlette.testclient import TestClient
        ...
        monkeypatch.setattr(
            local_bridge,
            "_marketplace_skill_wizard_chat",
            lambda body: ({"ok": True, "reply": "next question", "draft": {}}, 200),
        )
        monkeypatch.setattr(
            local_bridge,
            "_create_marketplace_skill_draft",
            lambda body: ({"ok": True, "draft": {"path": "alpha"}}, 200),
        )
        monkeypatch.setattr(
            local_bridge,
            "_publish_marketplace_skill_draft",
            lambda body: ({"ok": True, "listingPath": "alpha.json"}, 200),
        )
        client = TestClient(app)
        headers = {"Origin": "https://skilzzz.com"}

        wizard = client.post("/api/skills/marketplace/wizard-chat", json={"messages": []}, headers=headers)
        create = client.post("/api/skills/marketplace/create-draft", json={"name": "alpha"}, headers=headers)
        publish = client.post("/api/skills/marketplace/publish-draft", json={"name": "alpha"}, headers=headers)

        assert wizard.status_code == 200
        assert create.status_code == 200
        assert publish.status_code == 200
        assert wizard.headers["access-control-allow-origin"] == "https://skilzzz.com"
        assert create.headers["access-control-allow-origin"] == "https://skilzzz.com"
        assert publish.headers["access-control-allow-origin"] == "https://skilzzz.com"

    def test_marketplace_skill_install_public(self, monkeypatch):
        ...
        monkeypatch.setattr(wallet_context, "build_wallet_context", lambda: SimpleNamespace(skill_licenses={"alpha": {}}))
        ...
        resp = TestClient(app).post("/api/skills/marketplace/install", json={"name": "alpha"})
        ...
        assert resp.status_code == 200
        assert resp.json()["installed"]["name"] == "alpha"
        assert resp.json()["status"]["status"] == "installed"

    def test_marketplace_skill_install_request_status_public(self):
        from hermes_cli.local_bridge import create_install_request
        from starlette.testclient import TestClient
        from hermes_cli.web_server import app

        request = create_install_request({"name": "alpha"}, origin="test")
        resp = TestClient(app).get(f"/api/skills/marketplace/install-requests/{request['id']}")

        assert resp.status_code == 200
        assert resp.json()["request"]["id"] == request["id"]
        assert resp.json()["request"]["status"] == "pending"

    def test_marketplace_skill_install_request_approve_public(self):
        from hermes_cli.local_bridge import create_install_request
        from starlette.testclient import TestClient
        from hermes_cli.web_server import app

        request = create_install_request({"name": "alpha"}, origin="test")
        resp = TestClient(app).post(f"/api/skills/marketplace/install-requests/{request['id']}/approve")

        assert resp.status_code == 200
        assert resp.json()["request"]["id"] == request["id"]
        assert resp.json()["request"]["status"] == "approved"
