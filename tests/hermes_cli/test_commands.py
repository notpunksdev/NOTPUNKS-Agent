...
class TestCommandRegistry:
    ...
    def test_every_entry_has_valid_category(self):
        valid_categories = {"Session", "Configuration", "Tools & Skills", "Wallet", "Info", "Exit"}
        for cmd in COMMAND_REGISTRY:
...
class TestSlackNativeSlashes:
    ...
    def test_notpunks_catchall_is_first(self):
        """``/notpunks`` must be reserved as the first slot so the
        ``/notpunks <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "notpunks"
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `notpunks slack manifest`)."""
...
class TestSubcommands:
    def test_explicit_subcommands_extracted(self):
        ...
        for sub in ("browse", "search", "install", "inspect", "create", "list", "check", "update", "audit", "uninstall", "delete", "reset", "publish", "snapshot", "tap"):
            assert sub in SUBCOMMANDS["/skills"]
    ...
    def test_reasoning_has_subcommands(self):
        ...
        assert "show" in subs
        assert "hide" in subs
        assert SUBCOMMANDS["/effort"] == subs
...
class TestSubcommandCompletion:
    ...
    def test_alias_subcommand_completion_after_space(self):
        """Typing '/effort ' then Tab should show reasoning subcommands."""
        completions = _completions(SlashCommandCompleter(), "/effort ")
        texts = {c.text for c in completions}
        assert "high" in texts
        assert "show" in texts
...
class TestGhostText:
    ...
    def test_alias_subcommand_suggestion(self):
        """/effort h → 'igh'"""
        assert _suggestion("/effort h") == "igh"
...
class TestPluginCommandEnumeration:
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/notpunks metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
