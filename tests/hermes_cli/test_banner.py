...
def test_glitch_logo_frames_end_with_stable_logo():
    frames = banner._glitch_logo_frames(banner.NOTPUNKS_AGENT_LOGO)

    assert len(frames) >= 2
    assert frames[-1] == banner.NOTPUNKS_AGENT_LOGO
    assert any(frame != banner.NOTPUNKS_AGENT_LOGO for frame in frames[:-1])
...
def test_build_welcome_banner_title_is_hyperlinked_to_release():
    ...
    assert "Agent v" in raw, "Version label missing from title"
