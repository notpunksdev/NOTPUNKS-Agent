...
from hermes_cli.skills_hub import do_check, do_create, do_delete, do_install, do_list, do_update, handle_skills_slash
...
def _allow_local_skill_create(monkeypatch, tmp_path):
    import tools.skill_manager_tool as skill_manager

    skills_dir = tmp_path / "skills"
    skills_dir.mkdir()
    monkeypatch.setattr(
        "agent.wallet.context.build_wallet_context",
        lambda: SimpleNamespace(can_create_skills=True),
    )
...
def test_do_create_noninteractive_creates_starter_skill(monkeypatch, tmp_path):
    skills_dir = _allow_local_skill_create(monkeypatch, tmp_path)
    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None)

    do_create("wallet-debug", category="custom", console=console, interactive=False)

    skill_md = skills_dir / "custom" / "wallet-debug" / "SKILL.md"
    assert skill_md.exists()
    content = skill_md.read_text()
    assert "name: wallet-debug" in content
    assert "## Steps" in content
    output = sink.getvalue()
    assert "Skill 'wallet-debug' created" in output
    assert "Starter SKILL.md created" in output


def test_do_create_noninteractive_without_name_prints_tui_usage(monkeypatch, tmp_path):
    _allow_local_skill_create(monkeypatch, tmp_path)
    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None)

    do_create("", console=console, interactive=False)

    output = sink.getvalue()
    assert "/skills create <name>" in output
    assert "notpunks skills create" in output


def test_handle_skills_slash_create_makes_starter_skill(monkeypatch, tmp_path):
    skills_dir = _allow_local_skill_create(monkeypatch, tmp_path)
    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None)

    handle_skills_slash("/skills create --category custom wallet-debug", console=console)

    skill_md = skills_dir / "custom" / "wallet-debug" / "SKILL.md"
    assert skill_md.exists()
    assert "name: wallet-debug" in skill_md.read_text()


def test_do_delete_removes_local_skill(monkeypatch, tmp_path):
    skills_dir = _allow_local_skill_create(monkeypatch, tmp_path)
    skill_dir = skills_dir / "custom" / "wallet-debug"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("---\nname: wallet-debug\ndescription: ok\n---\n# ok\n")
    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None)

    do_delete("wallet-debug", console=console, skip_confirm=True)

    assert not skill_dir.exists()
    assert "deleted" in sink.getvalue()


def test_handle_skills_slash_delete_removes_local_skill(monkeypatch, tmp_path):
    skills_dir = _allow_local_skill_create(monkeypatch, tmp_path)
    skill_dir = skills_dir / "custom" / "wallet-debug"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("---\nname: wallet-debug\ndescription: ok\n---\n# ok\n")
    ...
    handle_skills_slash("/skills delete wallet-debug", console=console)
