...
def test_setup_summary_uses_notpunks_start_command(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    features = SimpleNamespace(
        web=SimpleNamespace(managed_by_nous=False, available=True, current_provider="Tavily"),
        browser=SimpleNamespace(managed_by_nous=False, available=True, current_provider="Local browser"),
        image_gen=SimpleNamespace(managed_by_nous=False, available=True),
        tts=SimpleNamespace(managed_by_nous=False),
        modal=SimpleNamespace(managed_by_nous=False, direct_override=False),
        nous_auth_present=False,
    )
    monkeypatch.setattr(setup_mod, "get_nous_subscription_features", lambda config: features)
    ...
    setup_mod._print_setup_summary({"tts": {"provider": "edge"}, "terminal": {}}, tmp_path)

    out = capsys.readouterr().out
    assert "notpunks" in out
    assert "Start chatting" in out
    assert "hermes              Start chatting" not in out
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    assert "notpunks gateway" in out
...
def test_resolve_hermes_chat_argv_prefers_which(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.shutil, "which", lambda name: "/usr/local/bin/notpunks" if name == "notpunks" else None)

    assert setup_mod._resolve_hermes_chat_argv() == ["/usr/local/bin/notpunks", "chat"]
...
def test_offer_launch_chat_execs_fresh_process(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "_resolve_hermes_chat_argv", lambda: ["/usr/local/bin/notpunks", "chat"])
    ...
    assert exec_calls == [("/usr/local/bin/notpunks", ["/usr/local/bin/notpunks", "chat"])]
...
def test_offer_launch_chat_manual_fallback_when_unresolvable(monkeypatch, capsys):
    ...
    assert "Run 'notpunks chat' manually" in captured.out
