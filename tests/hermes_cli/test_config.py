...
class TestGetHermesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False), patch.object(Path, "home", return_value=Path("/tmp/notpunks-test-home")):
            os.environ.pop("HERMES_HOME", None)
            os.environ.pop("NOTPUNKS_HOME", None)
            home = get_hermes_home()
            assert home == Path.home() / ".notpunks"
...
class TestLoadConfigDefaults:
    def test_returns_defaults_when_no_file(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert config["provider_routing"]["sort"] == "throughput"
