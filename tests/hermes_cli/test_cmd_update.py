...
from hermes_cli.main import (
    PROJECT_ROOT,
    _is_fork,
    _upstream_points_to_legacy_hermes,
    cmd_update,
)
...
class TestOfficialRepoDetection:
    @pytest.mark.parametrize(
        "remote_url",
        [
            "https://github.com/notpunksdev/NOTPUNKS-Agent.git",
            "https://github.com/notpunksdev/NOTPUNKS-Agent",
            "git@github.com:notpunksdev/NOTPUNKS-Agent.git",
            "git@github.com-notpunksdev:notpunksdev/NOTPUNKS-Agent.git",
        ],
    )
    def test_notpunks_origin_is_not_treated_as_fork(self, remote_url):
        assert _is_fork(remote_url) is False

    def test_other_origin_is_treated_as_fork(self):
        assert _is_fork("git@github.com:someone/NOTPUNKS-Agent.git") is True

    @patch("subprocess.run")
    def test_legacy_hermes_upstream_is_detected(self, mock_run):
        ...
        assert _upstream_points_to_legacy_hermes(["git"], PROJECT_ROOT) is True
