...
class TestCLIStatusBar:
    ...
    def test_status_brand_glitches_periodically_with_fixed_width(self):
        assert HermesCLI._status_brand_glitch_text(now=0.1) != "NOTPUNKS"
        assert HermesCLI._status_brand_glitch_text(now=1.2) == "NOTPUNKS"
        assert len(HermesCLI._status_brand_glitch_text(now=0.1)) == len("NOTPUNKS")

    def test_status_brand_glitch_can_be_disabled(self, monkeypatch):
        monkeypatch.setenv("HERMES_BANNER_GLITCH", "0")
        assert HermesCLI._status_brand_glitch_text(now=0.1) == "NOTPUNKS"
        assert HermesCLI._status_brand_glitch_style(now=0.1) == "class:status-bar-brand"
...
class TestStatusBarWidthSource:
    ...
    def test_wide_fragments_include_live_notpunks_brand(self):
        from unittest.mock import MagicMock, patch
        cli_obj = self._make_wide_cli()
        mock_app = MagicMock()
        ...
        with patch("prompt_toolkit.application.get_app", return_value=mock_app), \
             patch.object(HermesCLI, "_status_brand_glitch_text", return_value="N0TPUNKS"):
            frags = cli_obj._get_status_bar_fragments()

        assert "N0TPUNKS" in "".join(text for _, text in frags)
