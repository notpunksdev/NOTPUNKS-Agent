...
class TestHandleReasoningCommand(unittest.TestCase):
    ...
    @patch("cli._cprint")
    def test_effort_level_updates_existing_agent_without_reset(self, _mock_cprint, _mock_save):
        """Changing effort should not force a slow agent re-init when one exists."""
        from cli import HermesCLI

        cli = HermesCLI.__new__(HermesCLI)
        cli.reasoning_config = {"enabled": True, "effort": "medium"}
        cli.show_reasoning = False
        cli.agent = SimpleNamespace(reasoning_config=cli.reasoning_config)
        cli._start_agent_prewarm = MagicMock()

        HermesCLI._handle_reasoning_command(cli, "/effort high")

        self.assertEqual(cli.reasoning_config, {"enabled": True, "effort": "high"})
        self.assertEqual(cli.agent.reasoning_config, {"enabled": True, "effort": "high"})
        cli._start_agent_prewarm.assert_not_called()
