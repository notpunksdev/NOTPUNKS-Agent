...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "notpunks skills reset new-skill" in captured

    def test_stale_unmodified_bundled_skill_removed_from_disk(self, tmp_path):
        """Removed bundled skills should disappear if the user never modified them."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"

        removed_skill = skills_dir / "research" / "removed-skill"
        removed_skill.mkdir(parents=True)
        (removed_skill / "SKILL.md").write_text("---\nname: removed-skill\n---\n# Removed\n")
        origin_hash = _dir_hash(removed_skill)
        ...
        assert "removed-skill" in result["cleaned"]
        assert "removed-skill" in result["stale_removed"]
        assert not removed_skill.exists()

    def test_stale_user_modified_bundled_skill_is_kept(self, tmp_path):
        """Removed bundled skills are kept if the local copy was customized."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ...
        assert "removed-skill" in result["cleaned"]
        assert "removed-skill" in result["stale_user_modified"]
