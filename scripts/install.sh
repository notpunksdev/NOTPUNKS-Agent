#!/bin/bash
# ============================================================================
# NOTPUNKS Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/notpunksdev/NOTPUNKS-Agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:notpunksdev/NOTPUNKS-Agent.git"
REPO_URL_HTTPS="https://github.com/notpunksdev/NOTPUNKS-Agent.git"
HERMES_HOME="${NOTPUNKS_HOME:-${HERMES_HOME:-$HOME/.notpunks}}"
export HERMES_HOME
export NOTPUNKS_HOME="${NOTPUNKS_HOME:-$HERMES_HOME}"
...
#   code at /usr/local/lib/notpunks-agent, command at /usr/local/bin/notpunks,
#   data still at /root/.notpunks (NOTPUNKS_HOME). Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dir)
            ...
            shift 2
            ;;
        --notpunks-home|--hermes-home)
            HERMES_HOME="$2"
            NOTPUNKS_HOME="$2"
            export HERMES_HOME NOTPUNKS_HOME
            shift 2
            ;;
        -h|--help)
            echo "NOTPUNKS Agent Installer"
            echo ""
            ...
            echo "                   default (non-root):  ~/.notpunks/notpunks-agent"
            echo "                   default (root, Linux): /usr/local/lib/notpunks-agent"
            echo "  --notpunks-home PATH  Data directory (default: ~/.notpunks, or \$NOTPUNKS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, NOTPUNKS installs the code under"
            echo "  /usr/local/lib/notpunks-agent and links the command into"
            echo "  /usr/local/bin/notpunks (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NOTPUNKS_HOME"
            echo "  (default /root/.notpunks). This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing legacy installs are preserved in-place."
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│           ◆ NOTPUNKS Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  Local-first AI agent with wallet-gated skills.         │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `notpunks` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/notpunks-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/notpunks-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/notpunks-agent
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/notpunks-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/hermes-agent/.git" ]; then
            ...
            log_info "  (new root installs use /usr/local/lib/notpunks-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/notpunks-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/notpunks"
...
get_hermes_command_path() {
    ...
    if [ -x "$link_dir/notpunks" ]; then
        echo "$link_dir/notpunks"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            OS="windows"
            DISTRO="windows"
            log_error "Windows detected. Please install through WSL2:"
            log_info "  wsl --install"
            log_info "  Then run the Linux install command inside Ubuntu/WSL."
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (NOTPUNKS-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    # Place into the NOTPUNKS data directory and symlink binaries to ~/.local/bin/
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "NOTPUNKS Agent itself does not require or retain root access."
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            git_no_proxy fetch origin
            git_no_proxy checkout "$BRANCH"
            git_no_proxy pull --ff-only origin "$BRANCH"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if NOTPUNKS behaves unexpectedly."
                    else
        ...
        if GIT_SSH_COMMAND="ssh -o BatchMode=yes -o ConnectTimeout=5" \
           git_no_proxy clone --branch "$BRANCH" "$REPO_URL_SSH" "$INSTALL_DIR" 2>/dev/null; then
            log_success "Cloned via SSH"
        else
            rm -rf "$INSTALL_DIR" 2>/dev/null  # Clean up partial SSH clone
            log_info "SSH failed, trying HTTPS..."
            if git_no_proxy clone --branch "$BRANCH" "$REPO_URL_HTTPS" "$INSTALL_DIR"; then
                log_success "Cloned via HTTPS"
            else
                log_error "Failed to clone repository"
                log_info "If your shell or Git config forces a proxy, retry after clearing it:"
                log_info "  unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY all_proxy"
                log_info "  git config --global --unset http.proxy 2>/dev/null || true"
                log_info "  git config --global --unset https.proxy 2>/dev/null || true"
                exit 1
            fi
        fi
    fi
...
setup_path() {
    log_info "Setting up notpunks command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/notpunks"
    else
        HERMES_BIN="$(which notpunks 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "notpunks not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "notpunks entry point not found at $HERMES_BIN"
    ...
    # Create user-facing shims. `notpunks` is the primary command; `hermes`
    # remains as a compatibility alias for upstream scripts and old installs.
    mkdir -p "$command_link_dir"
    ln -sf "$HERMES_BIN" "$command_link_dir/notpunks"
    ln -sf "$HERMES_BIN" "$command_link_dir/hermes"
    log_success "Symlinked notpunks → $command_link_display_dir/notpunks"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "notpunks command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v notpunks' \
                >/dev/null 2>&1; then
        ...
        log_info "notpunks not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# NOTPUNKS Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# NOTPUNKS Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# NOTPUNKS Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so notpunks works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create NOTPUNKS directory structure (config at top level, code in subdir)
    mkdir -p "$HERMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env in the NOTPUNKS data directory (top level, easy to find)
    ...
    # Create config.yaml in the NOTPUNKS data directory (top level, easy to find)
...
# NOTPUNKS Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how NOTPUNKS communicates with you.
...
SOUL_EOF
    ...
    # Seed bundled skills into the NOTPUNKS data directory (manifest-based, one-time per skill)
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — NOTPUNKS itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (notpunks --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'notpunks setup' after install."
        return 0
    fi
    ...
    # Run NOTPUNKS setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for NOTPUNKS to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'notpunks whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'notpunks whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'notpunks gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: notpunks gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: notpunks gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            log_info "To restart later: notpunks gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: notpunks gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}notpunks${NC}              Start chatting"
    echo -e "   ${GREEN}notpunks setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}notpunks config${NC}       View/edit configuration"
    echo -e "   ${GREEN}notpunks config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}notpunks gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}notpunks update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'notpunks' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'notpunks' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'notpunks' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
