...
describe('createSlashHandler', () => {
  ...
  it('starts an agent-driven wizard for bare /skills create', () => {
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        rpc: vi.fn(() => Promise.resolve({ value: 'en' }))
      }
    })

    expect(createSlashHandler(ctx)('/skills create')).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('config.get', { key: 'language' })
  })

  it('uses Russian wizard prompt when /language is ru', async () => {
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        rpc: vi.fn(() => Promise.resolve({ value: 'ru' }))
      }
    })

    createSlashHandler(ctx)('/skills create')
    await Promise.resolve()
    expect(ctx.transcript.send).toHaveBeenCalledWith(expect.stringContaining('Задавай вопросы на русском языке'))
  })

  it('forwards /skills create <name> to the skills dispatcher', () => {
    const ctx = buildCtx()

    expect(createSlashHandler(ctx)('/skills create wallet-debug --category custom')).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('skills.manage', {
      action: 'command',
      command: '/skills create wallet-debug --category custom'
    })
  })

  it('forwards maintenance /skills subcommands to the skills dispatcher', () => {
    const ctx = buildCtx()

    expect(createSlashHandler(ctx)('/skills audit wallet-debug')).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('skills.manage', {
      action: 'command',
      command: '/skills audit wallet-debug'
    })
  })

  it('forwards /skills delete <name> to the skills dispatcher', () => {
    const ctx = buildCtx()

    expect(createSlashHandler(ctx)('/skills delete wallet-debug')).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('skills.manage', {
      action: 'command',
      command: '/skills delete wallet-debug'
    })
  })

  it('routes /language ru to config.set', () => {
    patchUiState({ sid: 'sid-abc' })
    const ctx = buildCtx()

    expect(createSlashHandler(ctx)('/language ru')).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('config.set', {
      key: 'language',
      session_id: 'sid-abc',
      value: 'ru'
