...
interface SkillsCommandResponse {
  output?: string
}

const SKILLS_USAGE =
  'usage: /skills [browse | search <q> | install <n> | inspect <n> | create <n> | list | check | update | audit | uninstall <n> | delete <n> | reset <n> | publish <path> | snapshot export|import | tap list|add|remove]'

const FORWARDED_SKILLS_SUBCOMMANDS = new Set([
  'create',
  'check',
  'update',
  'audit',
...
export const opsCommands: SlashCommand[] = [
  ...
  {
    help: 'browse, search, install, create, or manage skills',
    name: 'skills',
    run: (arg, ctx) => {
      const text = arg.trim()
      ...
      const { panel, send, sys } = ctx.transcript
      ...
      if (sub === 'create' && !query) {
        const englishWizard = [
          ...
          'Ask for the skill name, optional category, when to use it, step-by-step instructions, pitfalls, and verification.',
          'Do not create the skill until you have enough information.',
          'When ready, create it with the skill management tool. NFT access is enforced by the tool.'
        ].join(' ')
        const russianWizard = [
          'Запусти интерактивный мастер создания NOTPUNKS skill.',
          'Задавай вопросы на русском языке: имя навыка, необязательная категория, когда его использовать, пошаговые инструкции, возможные ошибки и проверка результата.',
          'Не создавай skill, пока не соберёшь достаточно информации.',
          'Когда всё готово, создай его через skill management tool. NFT-доступ проверяется самим tool.'
        ].join(' ')

        rpc<ConfigGetValueResponse>('config.get', { key: 'language' })
          .then(ctx.guarded<ConfigGetValueResponse>(r => send(r.value === 'ru' ? russianWizard : englishWizard)))
          .catch(() => send(englishWizard))

        return
      }

      if (FORWARDED_SKILLS_SUBCOMMANDS.has(sub ?? '')) {
        rpc<SkillsCommandResponse>('skills.manage', { action: 'command', command: `/skills ${text}` })
          .then(
            ctx.guarded<SkillsCommandResponse>(r => {
              const output = String(r.output ?? '').trim()
              sys(output || `skills ${sub} complete`)
            })
          )
          .catch(ctx.guardedErr)
