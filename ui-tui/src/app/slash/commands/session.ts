...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    aliases: ['lang'],
    help: 'set response language [en|ru|status]',
    name: 'language',
    run: (arg, ctx) => {
      const mode = arg.trim().toLowerCase()
      const valid = new Set(['', 'status', 'en', 'ru', 'english', 'russian', 'eng', 'rus', 'английский', 'русский'])

      if (!valid.has(mode)) {
        return ctx.transcript.sys('usage: /language [en|ru|status]')
      }

      if (!mode || mode === 'status') {
        return ctx.gateway
          .rpc<ConfigGetValueResponse>('config.get', { key: 'language' })
          .then(ctx.guarded<ConfigGetValueResponse>(r => ctx.transcript.sys(`language: ${r.label || r.value || 'English'}`)))
          .catch(ctx.guardedErr)
      }

      ctx.gateway
        .rpc<ConfigSetResponse>('config.set', { key: 'language', session_id: ctx.sid, value: mode })
        .then(
          ctx.guarded<ConfigSetResponse>(r => {
            if (r.history_reset) {
              ctx.session.resetVisibleHistory(r.info ?? null)
            }
            ctx.transcript.sys(`language: ${r.label || r.value}`)
            ctx.transcript.sys(
              r.history_reset
                ? 'language applied. transcript cleared.'
                : 'start a new session for the global system prompt to fully apply'
            )
          })
        )
        .catch(ctx.guardedErr)
