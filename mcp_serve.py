"""
NOTPUNKS MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (NOTPUNKS-specific extra)

Usage:
    notpunks mcp serve
    notpunks mcp serve --verbose
...
class EventBridge:
    ...
    This is the NOTPUNKS equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the NOTPUNKS MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "NOTPUNKS Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the NOTPUNKS MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
