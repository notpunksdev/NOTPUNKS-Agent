...
The ACP adapter wraps NOTPUNKS' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> NOTPUNKS `once`
- `allow_always` -> NOTPUNKS `always`
- reject options -> NOTPUNKS `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps NOTPUNKS tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses NOTPUNKS' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured NOTPUNKS provider/credentials.
