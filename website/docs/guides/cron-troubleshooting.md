...
description: "Diagnose and fix common NOTPUNKS cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the NOTPUNKS config directory). Verify:
```bash
...
hermes logs                         # View recent NOTPUNKS logs
