---
sidebar_position: 7
title: "Use SOUL.md with NOTPUNKS"
description: "How to use SOUL.md to shape NOTPUNKS Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with NOTPUNKS

`SOUL.md` is the **primary identity** for your NOTPUNKS instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want NOTPUNKS to feel like the same assistant every time you talk to it — or if you want to replace the NOTPUNKS persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm NOTPUNKS should be
- what NOTPUNKS should avoid stylistically
- how NOTPUNKS should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who NOTPUNKS is and how NOTPUNKS speaks
...
NOTPUNKS now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run NOTPUNKS with a custom home directory, it becomes:
...
NOTPUNKS automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, NOTPUNKS does not overwrite it
- if the file exists but is empty, NOTPUNKS adds nothing from it to the prompt

## How NOTPUNKS uses it

When NOTPUNKS starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how NOTPUNKS feels.
...
NOTPUNKS already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How NOTPUNKS should sound.

# Avoid
What NOTPUNKS should not do.

# Defaults
How NOTPUNKS should behave when ambiguity appears.
```
...
Then restart NOTPUNKS or start a new session.
...
4. Talk to NOTPUNKS for a while
...
### I edited SOUL.md but NOTPUNKS still sounds the same
...
### NOTPUNKS is ignoring parts of my SOUL.md
