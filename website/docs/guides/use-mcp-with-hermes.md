---
sidebar_position: 6
title: "Use MCP with NOTPUNKS"
description: "A practical guide to connecting MCP servers to NOTPUNKS Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with NOTPUNKS

This guide shows how to actually use MCP with NOTPUNKS Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native NOTPUNKS tool
- you want NOTPUNKS to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect NOTPUNKS to internal APIs, databases, or company systems without modifying NOTPUNKS core

Do not use MCP when:
- a built-in NOTPUNKS tool already solves the job well
...
- NOTPUNKS remains the agent
- MCP servers contribute tools
- NOTPUNKS discovers those tools at startup or reload time
...
If you installed NOTPUNKS with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- NOTPUNKS banner/status should show MCP integration when configured
- ask NOTPUNKS what tools it has available
...
There are two categories of MCP-exposed functionality in NOTPUNKS:
...
2. NOTPUNKS-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So NOTPUNKS will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want NOTPUNKS to reason over a bounded workspace.
...
Now NOTPUNKS can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing NOTPUNKS core.
...
Because NOTPUNKS now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
