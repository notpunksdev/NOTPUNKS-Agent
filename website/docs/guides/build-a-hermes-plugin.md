...
title: "Build a NOTPUNKS Plugin"
description: "Step-by-step guide to building a complete NOTPUNKS plugin with tools, hooks, data files, and skills"
---

# Build a NOTPUNKS Plugin

This guide walks through building a complete NOTPUNKS plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
This tells NOTPUNKS: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** NOTPUNKS may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but NOTPUNKS continues fine
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), NOTPUNKS injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is NOTPUNKS's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
```python
# Wrong — will break if NOTPUNKS passes extra context
def handler(args):
    ...
