...
description: "Authoritative reference for NOTPUNKS terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which NOTPUNKS profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes backup` | Back up NOTPUNKS home directory to a zip file. |
| `hermes import` | Restore a NOTPUNKS backup from a zip file. |
...
| `hermes acp` | Run NOTPUNKS as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run NOTPUNKS as an MCP server. |
| `hermes plugins` | Manage NOTPUNKS Agent plugins (install, enable, disable, remove). |
| `hermes tools` | Configure enabled tools per platform. |
| `hermes sessions` | Browse, export, prune, rename, and delete sessions. |
| `hermes insights` | Show token/cost/activity analytics. |
| `hermes claw` | OpenClaw migration helpers. |
| `hermes dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `hermes profile` | Manage profiles — multiple isolated NOTPUNKS instances. |
...
| `hermes uninstall` | Remove NOTPUNKS from the system. |
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active NOTPUNKS chat session.
...
**`hermes model`** (run from your terminal, outside any NOTPUNKS session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active NOTPUNKS chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your NOTPUNKS session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `--name NAME` | `NOTPUNKS` | Bot display name in Slack. |
...
Outputs a compact, plain-text summary of your entire NOTPUNKS setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | NOTPUNKS version, release date, git commit hash |
...
The report includes system info (OS, Python version, NOTPUNKS version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Create a zip archive of your NOTPUNKS configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when NOTPUNKS is running (WAL-mode safe).
...
Restore a previously created NOTPUNKS backup into your NOTPUNKS home directory.
...
View, tail, and filter NOTPUNKS log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
NOTPUNKS uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
| `install-marketplace` | Install from the NOTPUNKS marketplace, preferring encrypted sNFT cartridges. |
| `install-snft` | Install directly from an sNFT metadata URL, or as an alias for marketplace install. |
...
hermes skills install-marketplace deploy-bot --force
hermes skills install-snft https://app.notpunks.com/api/skills/marketplace/deploy-bot/metadata --force
hermes skills check
...
- `--source well-known` lets you point NOTPUNKS at a site exposing `/.well-known/skills/index.json`.
- sNFT installs select TON by default. Use `NOTPUNKS_SNFT_CHAIN=evm` or `NOTPUNKS_SNFT_CHAIN=solana` to prefer another adapter.
- Interactive EVM/Solana sNFT installs open a temporary local browser signer; automation can provide `NOTPUNKS_SNFT_UNLOCK_REQUEST` or `NOTPUNKS_SNFT_UNLOCK_REQUEST_FILE`.
...
| `migrate` | Step-by-step migration guide from openclaw-honcho to NOTPUNKS Honcho. |
...
Starts NOTPUNKS as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run NOTPUNKS as an MCP server.

| Subcommand | Description |
|------------|-------------|
| `serve [-v\|--verbose]` | Run NOTPUNKS as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with NOTPUNKS](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a NOTPUNKS Plugin](../guides/build-a-hermes-plugin.md).
...
Migrate your OpenClaw setup to NOTPUNKS. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing NOTPUNKS files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into NOTPUNKS equivalents or **archived** for manual review.
...
Manage profiles — multiple isolated NOTPUNKS instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of NOTPUNKS commands, subcommands, and profile names.
...
| `hermes uninstall [--full] [--yes]` | Remove NOTPUNKS, optionally deleting all config/data. |
