...
description: "Complete reference of all environment variables used by NOTPUNKS Agent"
---
...
| `MINIMAX_BASE_URL` | Override MiniMax base URL (default: `https://api.minimax.io/anthropic` — NOTPUNKS uses MiniMax's Anthropic Messages-compatible endpoint) |
...
| `HERMES_HOME` | Override NOTPUNKS config directory (default: `~/.hermes`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, NOTPUNKS prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.
...
## NOTPUNKS Marketplace

| Variable | Description |
|----------|-------------|
| `NOTPUNKS_MARKETPLACE_PUBLISH_TOKEN` | Admin token used to publish/unpublish marketplace skills and as a fallback for sNFT cartridge unlocks |
| `SKILL_MARKETPLACE_PUBLISH_SECRET` | Legacy alias for `NOTPUNKS_MARKETPLACE_PUBLISH_TOKEN` |
| `NOTPUNKS_MARKETPLACE_UNLOCK_TOKEN` | Optional issuer/admin token used by `/skills install-snft` and the reference verifier to unlock encrypted sNFT cartridges without a wallet proof |
| `NOTPUNKS_SNFT_CHAIN` | Preferred sNFT unlock adapter chain for installs: `ton`, `evm`, or `solana` |
| `NOTPUNKS_SNFT_UNLOCK_REQUEST` | Pre-signed normalized sNFT unlock request JSON. Used for automation; interactive EVM/Solana installs open the local browser signer when this is unset |
| `NOTPUNKS_SNFT_UNLOCK_REQUEST_FILE` | Path to a pre-signed normalized sNFT unlock request JSON file. Used for automation; interactive EVM/Solana installs open the local browser signer when this is unset |
| `SKILL_MARKETPLACE_DEV_LICENSE_FALLBACK` | Development-only flag that lets wallet/license checks trust local `mint_intents` records when a real Skill NFT is not visible on-chain |
| `SKILL_MARKETPLACE_DEV_MINT` | Development-only flag for mock Skill NFT purchase/mint flows; also enables local mint-intent license fallback |
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when NOTPUNKS cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `GATEWAY_PROXY_URL` | URL of a remote NOTPUNKS API server to forward messages to ([proxy mode](/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
For task-specific direct endpoints, NOTPUNKS uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
