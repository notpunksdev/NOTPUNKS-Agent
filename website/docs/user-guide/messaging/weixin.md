...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect NOTPUNKS Agent to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect NOTPUNKS to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
- Terminal QR rendering is included when NOTPUNKS is installed with the `messaging` extra
...
| `Another local NOTPUNKS gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
