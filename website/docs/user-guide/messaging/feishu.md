...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up NOTPUNKS Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

NOTPUNKS Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; NOTPUNKS opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | NOTPUNKS responds to every message. |
| Group chats | NOTPUNKS responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. NOTPUNKS will automatically create a bot application with the correct permissions and save the credentials.
...
Use WebSocket mode when NOTPUNKS runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run NOTPUNKS behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, NOTPUNKS starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how NOTPUNKS responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | NOTPUNKS responds to @mentions from any user in any group. |
| `allowlist` | NOTPUNKS only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | NOTPUNKS ignores all group messages entirely. |
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, NOTPUNKS reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
| `Another local NOTPUNKS gateway is already using this Feishu app_id` | Only one NOTPUNKS instance can use the same app_id at a time. Stop the other gateway first. |
