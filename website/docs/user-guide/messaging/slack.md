...
sidebar_position: 4
title: "Slack"
description: "Set up NOTPUNKS Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect NOTPUNKS Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your NOTPUNKS instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. NOTPUNKS uses the modern
...
The fastest path is to paste a manifest NOTPUNKS generates for you. It
...
### Option A: From a NOTPUNKS-generated manifest (recommended)
...
4. Enter an app name (e.g., "NOTPUNKS Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, NOTPUNKS can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a NOTPUNKS configuration issue.
:::
...
NOTPUNKS uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every NOTPUNKS command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
NOTPUNKS command with its description.

Under the hood: NOTPUNKS ships with a generated Slack app manifest (see
...
When NOTPUNKS adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — NOTPUNKS routes it the same way as `/btw
...
Understanding how NOTPUNKS behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@NOTPUNKS Agent what time is it?`). In channels, NOTPUNKS replies in a thread attached to that message. |
| **Threads** | If you @mention NOTPUNKS inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to NOTPUNKS in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where NOTPUNKS will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @NOTPUNKS Agent`).

---

## Multi-Workspace Support

NOTPUNKS can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, NOTPUNKS also loads tokens from an **OAuth token file** at:
...
- When a message arrives, NOTPUNKS uses the correct workspace-specific client to respond.
...
NOTPUNKS supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @NOTPUNKS Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @NOTPUNKS Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. NOTPUNKS now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @NOTPUNKS Agent`)
...
- Audit who has access to your NOTPUNKS config directory
