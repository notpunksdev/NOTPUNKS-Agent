...
Connect NOTPUNKS to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between NOTPUNKS and the BlueBubbles server
...
When someone messages your iMessage, NOTPUNKS automatically sends them a pairing code. Approve it with:
```bash
...
NOTPUNKS will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → NOTPUNKS
NOTPUNKS → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** NOTPUNKS sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — NOTPUNKS resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
