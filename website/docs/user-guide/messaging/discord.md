...
sidebar_position: 3
title: "Discord"
description: "Set up NOTPUNKS Agent as a Discord bot"
---

# Discord Setup

NOTPUNKS Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the NOTPUNKS Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how NOTPUNKS behaves once it's in your server.

## How NOTPUNKS Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | NOTPUNKS responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, NOTPUNKS only responds when you `@mention` it. If you post in a channel without mentioning it, NOTPUNKS ignores the message. |
...
| **Threads** | NOTPUNKS replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, NOTPUNKS isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), NOTPUNKS stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to NOTPUNKS without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

NOTPUNKS on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal NOTPUNKS agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and NOTPUNKS session policy.
...
So if Alice and Bob both talk to NOTPUNKS in `#research`, NOTPUNKS treats those as separate conversations by default even though they are using the same visible Discord channel.
...
NOTPUNKS tracks running agents by session key.
...
3. Enter a name for your application (e.g., "NOTPUNKS Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential NOTPUNKS Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the NOTPUNKS gateway).

## Step 7: Find Your Discord User ID

NOTPUNKS Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
## Step 8: Configure NOTPUNKS Agent
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` NOTPUNKS. `"all"` — accept all bot messages. |
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, NOTPUNKS falls back to the parent channel/forum ID.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with NOTPUNKS. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
NOTPUNKS automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.
...
NOTPUNKS Agent supports Discord voice messages:
...
- **Discord voice channels**: NOTPUNKS can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/docs/user-guide/features/voice-mode)
- [Use Voice Mode with NOTPUNKS](/docs/guides/use-voice-mode-with-hermes)

## Forum Channels

Discord forum channels (type 15) don't accept direct messages — every post in a forum must be a thread. NOTPUNKS auto-detects forum channels and creates a new thread post whenever it needs to send there, so `send_message`, TTS, images, voice messages, and file attachments all work without special handling from the agent.
...
**Cause**: The NOTPUNKS gateway isn't running, or the token is incorrect.
...
For more information on securing your NOTPUNKS Agent deployment, see the [Security Guide](../security.md).
