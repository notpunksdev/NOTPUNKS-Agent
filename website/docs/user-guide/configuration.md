...
sidebar_position: 2
title: "Configuration"
description: "Configure NOTPUNKS Agent — config.yaml, providers, models, API keys, and more"
---
...
NOTPUNKS supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means NOTPUNKS will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
**Requirements:** Docker Desktop or Docker Engine installed and running. NOTPUNKS probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).

**Container lifecycle:** NOTPUNKS reuses a single long-lived container (`docker run -d ... sleep 2h`) for every terminal and file-tool call, across sessions, `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the NOTPUNKS process. Commands run via `docker exec` with a login shell, so working-directory changes, installed packages, and files in `/workspace` all persist from one tool call to the next. The container is stopped and removed on NOTPUNKS shutdown (or when the idle-sweep reclaims it).
...
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. NOTPUNKS logs a clear error if either is missing.
...
NOTPUNKS resolves each listed variable from your current shell first, then falls back to `~/.hermes/.env` if it was saved with `hermes config set`.
...
Docker sandboxes stay isolated by default. NOTPUNKS does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch NOTPUNKS from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched NOTPUNKS from
...
Three related caps control how much raw output a tool can return before NOTPUNKS truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, NOTPUNKS keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
NOTPUNKS automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
NOTPUNKS has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.
...
The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so NOTPUNKS raises this to 30 minutes when it detects a local endpoint. If you explicitly set `HERMES_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default NOTPUNKS disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `HERMES_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
NOTPUNKS uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="NOTPUNKS Agent — Auxiliary Models Tutorial"
...
Every model slot in NOTPUNKS — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, NOTPUNKS ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, NOTPUNKS uses that provider's built-in auth and base URL.
...
If you use a reasoning-heavy model for `auxiliary.session_search`, NOTPUNKS now gives you two built-in controls:

- `auxiliary.session_search.max_concurrency`: limits how many matched sessions NOTPUNKS summarizes at once
...
Use `extra_body` only when your provider documents OpenAI-compatible request-body fields you want NOTPUNKS to pass through for that task. NOTPUNKS forwards the object as-is.

:::warning
`extra_body` is only effective when your provider actually supports the field you send. If the provider does not expose a native OpenAI-compatible reasoning-off flag, NOTPUNKS cannot synthesize one on its behalf.
:::
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, NOTPUNKS uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider NOTPUNKS uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
```yaml
agent:
  reasoning_effort: "low"   # default. Options: none, minimal, low, medium, high, xhigh (max)
```

NOTPUNKS defaults to `"low"` so fresh installs keep model reasoning enabled while staying responsive. Setting a higher value enables deeper model thinking for complex tasks at the cost of more tokens and latency. Use `"none"` only when raw speed matters more than answer quality.
...
display:
  ...
  streaming: true         # Stream tokens to terminal as they arrive (real-time output)
...
`interim_assistant_messages` is gateway-only. When enabled, NOTPUNKS sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, NOTPUNKS falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
CLI streaming is enabled by default for new installs. Set `display.streaming: false` in `~/.hermes/config.yaml` if you prefer to render only completed responses.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want NOTPUNKS to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. NOTPUNKS still keys DMs by chat/DM ID as usual.
...
Control what NOTPUNKS does when an unknown user sends a direct message:
...
- `pair` is the default. NOTPUNKS denies access, but replies with a one-time pairing code in DMs.
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (NOTPUNKS's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search`, `web_extract`, and `web_crawl` tools support the shipped backend providers below. Configure the backend in `config.yaml` or via `hermes tools`:
...
**Backend selection:** If `web.backend` is not set, the backend is auto-detected from available API keys. If only `EXA_API_KEY` is set, Exa is used. If only `PARALLEL_API_KEY` is set, Parallel is used. Otherwise Firecrawl is the default.
...
browser:
  ...
  # Optional CDP override — when set, NOTPUNKS attaches directly to your own
...
The browser toolset supports multiple providers. See the [Browser feature page](/docs/user-guide/features/browser) for details on Browserbase, Firecrawl, Camofox, and local Chrome CDP setup.
...
Control how NOTPUNKS handles potentially dangerous commands:
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, NOTPUNKS falls back to `OPENAI_API_KEY` only.
...
NOTPUNKS uses two different context scopes:
...
- If SOUL.md is missing, empty, or cannot be loaded, NOTPUNKS falls back to a built-in default identity.
...
- NOTPUNKS automatically seeds a default `SOUL.md` if one does not already exist.
