...
NOTPUNKS Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
When you resume a session, NOTPUNKS displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a NOTPUNKS session." />
...
When a session's context is compressed (manually via `/compress` or automatically), NOTPUNKS creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:
...
NOTPUNKS provides a full set of session management commands via `hermes sessions`:
...
When NOTPUNKS cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, NOTPUNKS uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to NOTPUNKS in the same Discord channel without sharing transcript history
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every NOTPUNKS process in the same `HERMES_HOME`
