...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for NOTPUNKS — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for NOTPUNKS — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run NOTPUNKS interactively.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while NOTPUNKS is starting.
...
On first launch NOTPUNKS installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new NOTPUNKS version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point NOTPUNKS at it:
...
The TUI respects all standard NOTPUNKS config: `~/.hermes/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
If the TUI fails to launch (no Node, missing bundle, TTY issue), NOTPUNKS prints a diagnostic and falls back — rather than leaving you stuck.
