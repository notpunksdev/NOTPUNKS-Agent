...
description: "Configure and use Honcho memory with NOTPUNKS -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
# Honcho Memory for NOTPUNKS

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every NOTPUNKS profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this NOTPUNKS profile's AI peer
...
Honcho models conversations as interactions between **peers**. NOTPUNKS creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this NOTPUNKS instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each NOTPUNKS run |
...
Each NOTPUNKS profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for NOTPUNKS when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all NOTPUNKS profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to NOTPUNKS + Honcho |
