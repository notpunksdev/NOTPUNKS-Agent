---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into NOTPUNKS Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into NOTPUNKS Agent"
---
...
Migrate a user's OpenClaw customization footprint into NOTPUNKS Agent. Imports NOTPUNKS-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | NOTPUNKS Agent (Nous Research) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `NOTPUNKS`, `Memory`, `Persona`, `Import` |
| Related skills | [`hermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) |

## Reference: full SKILL.md

:::info
The following is the complete skill definition that NOTPUNKS loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.
:::

# OpenClaw -> NOTPUNKS Migration

Use this skill when a user wants to move their OpenClaw setup into NOTPUNKS Agent with minimal manual cleanup.
...
- import `SOUL.md` into the NOTPUNKS home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into NOTPUNKS memory entries
- merge OpenClaw command approval patterns into NOTPUNKS `command_allowlist`
- migrate NOTPUNKS-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen NOTPUNKS workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct NOTPUNKS destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of NOTPUNKS-compatible secrets, currently:
...
NOTPUNKS CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in NOTPUNKS. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a NOTPUNKS workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing NOTPUNKS skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of NOTPUNKS unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty NOTPUNKS targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean NOTPUNKS destination. Unsupported auth blobs must still be reported as skipped.
...
- NOTPUNKS persona state imported
- NOTPUNKS memory files populated with converted OpenClaw knowledge
