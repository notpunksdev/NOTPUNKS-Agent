...
description: "Customize NOTPUNKS Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

NOTPUNKS Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who NOTPUNKS is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

NOTPUNKS now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run NOTPUNKS with a custom home directory, it will use:
...
- NOTPUNKS creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- NOTPUNKS loads `SOUL.md` only from `HERMES_HOME`
- NOTPUNKS does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, NOTPUNKS falls back to a built-in default identity
...
If NOTPUNKS loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the NOTPUNKS instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change NOTPUNKS' default personality."
...
- how NOTPUNKS should handle uncertainty, disagreement, or ambiguity
...
## What NOTPUNKS injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, NOTPUNKS falls back to a built-in default identity ("You are NOTPUNKS Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
NOTPUNKS ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain NOTPUNKS, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives NOTPUNKS its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how NOTPUNKS speaks
- `display.skin` and `/skin` affect how NOTPUNKS looks in the terminal
