...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, NOTPUNKS Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
```yaml
provider_routing:
  sort: "throughput"      # How to rank providers
...
New NOTPUNKS installs set `provider_routing.sort: "throughput"` by default so interactive agent sessions prefer faster token generation on OpenRouter. Existing configs without a `provider_routing` section continue to let OpenRouter use its own default routing logic until you add the section manually.
