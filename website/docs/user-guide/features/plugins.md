...
title: "Plugins"
description: "Extend NOTPUNKS with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

NOTPUNKS has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a NOTPUNKS Plugin](/docs/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.
...
Start NOTPUNKS — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal NOTPUNKS plugin — registers a tool and a hook."""
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart NOTPUNKS, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting NOTPUNKS.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with NOTPUNKS — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
...
When you upgrade to a version of NOTPUNKS that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.hermes/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled plugins are NOT grandfathered — even existing users have to opt in explicitly.
...
NOTPUNKS has three kinds of plugins:
