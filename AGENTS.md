# NOTPUNKS Agent - Development Guide
...
- `default` — Classic NOTPUNKS gold/kawaii (the current look)
...
NOTPUNKS has two plugin surfaces. Both live under `plugins/` in the repo so
...
NOTPUNKS-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
NOTPUNKS supports **profiles** — multiple fully isolated instances, each with its own
