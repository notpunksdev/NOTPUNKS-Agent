...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `notpunks auth
    ...
    raise SystemExit(f"`notpunks auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `notpunks auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `notpunks auth` is called bare."""
