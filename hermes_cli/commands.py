...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of NOTPUNKS config/state", "Session",
    ...
    CommandDef("personality", "Set a predefined personality", "Configuration",
               args_hint="[name]"),
    CommandDef("language", "Set response language", "Configuration",
               aliases=("lang",), args_hint="[en|ru|status]",
               subcommands=("en", "ru", "status")),
    ...
    CommandDef("busy", "Control what Enter does while NOTPUNKS is working", "Configuration",
    ...
    CommandDef("skills", "Search, install, inspect, or manage skills",
               ...
               subcommands=(
                   "browse", "search", "install", "inspect", "create", "list",
                   "check", "update", "audit", "uninstall", "delete", "reset",
                   "publish", "unpublish", "install-marketplace", "install-snft", "marketplace",
                   "marketplace-status",
                   "store", "access", "buy", "licenses", "earnings",
                   "snapshot", "tap",
               )),
    ...
    CommandDef("wallet", "TON wallet commands", "Wallet",
               cli_only=True, aliases=("w",),
               subcommands=("connect", "status", "disconnect", "nft", "access")),
    CommandDef("nft", "Scan NFTs and show unlocked skill modes", "Wallet",
               cli_only=True, aliases=("nfts",)),
    ...
    CommandDef("update", "Update NOTPUNKS Agent to the latest version", "Info",
               gateway_only=True),
...
for _cmd in COMMAND_REGISTRY:
    ...
    if m:
        ...
        for _alias in _cmd.aliases:
            SUBCOMMANDS[f"/{_alias}"] = list(SUBCOMMANDS[key])
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/notpunks`` subcommand mapping, and
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``notpunks skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/notpunks <verb>`` subcommand.
    ...
    avoidance. ``/notpunks`` is always reserved as the first entry so the
    catch-all ``/notpunks <subcommand>`` form keeps working for anything that
    ...
    # Reserve /notpunks as the catch-all top-level command.
    entries.append(("notpunks", "Talk to NOTPUNKS or run a subcommand", "[subcommand] [args]"))
    seen.add("notpunks")
...
def slack_app_manifest(request_url: str = "https://agent.notpunks.com/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /notpunks handler.

    Maps both canonical names and aliases so /notpunks bg do stuff works
    the same as /notpunks background do stuff.

    Plugin-registered slash commands are included so ``/notpunks <plugin-cmd>``
