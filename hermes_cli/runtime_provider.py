...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = (
        os.getenv("NOTPUNKS_INFERENCE_PROVIDER", "").strip().lower()
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'notpunks doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'notpunks model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "the NOTPUNKS .env file or run 'notpunks model' to configure."
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `notpunks auth list`).  If the key is
