...
Usage::
    ...
    notpunks profile create coder          # fresh profile + bundled skills
    notpunks profile create coder --clone  # also copy config, .env, SOUL.md
    notpunks profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hermes -p coder chat                 # or via flag
    notpunks profile use coder             # set as sticky default
    notpunks profile delete coder          # remove profile + alias + service
"""
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            ...
            "To remove everything, use: notpunks uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            ...
            f"Create it with: notpunks profile create {name}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: notpunks profile import <archive> --name <name>"
        )
    ...
    if inferred_name == "default":
        raise ValueError(
            ...
            "Specify a different name: notpunks profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for notpunks profile names."""
    return '''# NOTPUNKS Agent profile completion
# Add to ~/.bashrc: eval "$(notpunks completion bash)"
...
def generate_zsh_completion() -> str:
    """Generate a zsh completion script for notpunks profile names."""
    return '''#compdef hermes
# NOTPUNKS Agent profile completion
# Add to ~/.zshrc: eval "$(notpunks completion zsh)"
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: notpunks profile create {profile_name}"
        )
