"""
NOTPUNKS Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep the NOTPUNKS data directory (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove NOTPUNKS Agent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# NOTPUNKS Agent" comment and following line
                if '# NOTPUNKS Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script(wrapper_paths: list[Path] | None = None):
    """Remove the notpunks command and legacy hermes alias if they exist."""
    if wrapper_paths is None:
        wrapper_paths = [
            Path.home() / ".local" / "bin" / "notpunks",
            Path.home() / ".local" / "bin" / "hermes",
            Path("/usr/local/bin/notpunks"),
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``notpunks gateway run`` processes
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + NOTPUNKS data directory (configs, data, logs)
    - Keep data: removes code but keeps NOTPUNKS data for future reinstall
    """
    ...
    print(color("│            ◆ NOTPUNKS Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL NOTPUNKS Agent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the NOTPUNKS Agent code but keep your configuration and data.")
    ...
    log_info("Removing notpunks command and legacy compatibility alias...")
    ...
    try:
        if project_root.exists():
            # If the install is inside the NOTPUNKS data directory, remove only the code subdir.
    ...
    # 5. Optionally remove NOTPUNKS data directory (and named profiles)
    if full_uninstall:
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/notpunksdev/NOTPUNKS-Agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using NOTPUNKS Agent! ◆")
    print()
