"""
Multi-provider authentication system for NOTPUNKS Agent.
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    # "nous" provider intentionally omitted — Nous Portal is disabled in NOTPUNKS
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `notpunks model` to re-authenticate."
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `notpunks logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'notpunks doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'notpunks model' for available providers, or run 'notpunks doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'notpunks model' to choose a "
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `notpunks auth list` / `notpunks status`."""
    try:
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `notpunks auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `notpunks auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `notpunks auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `notpunks auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    # Persist so subsequent `notpunks auth spotify` runs skip the wizard.
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `notpunks auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `notpunks auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `notpunks auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `notpunks auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `notpunks auth` to re-authenticate."
            )
            relogin_required = True
    ...
    *,
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "refresh endpoint. For health checks, use `notpunks auth status` "
            "instead.\n"
            "Re-authenticate with: notpunks auth add nous"
        )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``notpunks auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``notpunks auth add nous --label <name>``).  It gets embedded in the
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Disabled in NOTPUNKS — Nous Portal is not a supported provider."""
    return {"logged_in": False, "disabled": True}


def _legacy_nous_auth_status_unused() -> Dict[str, Any]:
    """Original Nous auth-status logic — kept inert for reference."""
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `notpunks auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `notpunks auth` and
    # `notpunks model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `notpunks logout` historically keyed off auth.json.active_provider only.
...
def login_command(args) -> None:
    """Deprecated: use 'notpunks model' or 'notpunks setup' instead."""
    print("The 'notpunks login' command has been removed.")
    print("Use 'notpunks auth' to manage credentials,")
    print("'notpunks model' to select a provider, or 'notpunks setup' for full setup.")
    raise SystemExit(0)
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `notpunks model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `notpunks model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or config_matches:
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `notpunks model` or configure an API key to use Hermes.")
    else:
