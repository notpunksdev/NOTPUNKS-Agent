"""``notpunks debug`` — debug tools for NOTPUNKS Agent.

Currently supports:
    notpunks debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    ...
    ran ``notpunks debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``notpunks debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``notpunks debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``notpunks debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `notpunks debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``notpunks debug share`` invocation added ~20 MB of resident Python
    ...
    ``notpunks debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"notpunks debug delete {url}"
...
def _capture_dump() -> str:
    """Run ``notpunks dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``notpunks dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  notpunks debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: notpunks debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'notpunks debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``notpunks debug`` call.
    ...
    # best-effort — any failure is swallowed so ``notpunks debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: notpunks debug <command>")
        print()
        print("Commands:")
