...
app = FastAPI(title="NOTPUNKS Agent", version=__version__)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``notpunks dashboard --tui``
...
async def auth_middleware(request: Request, call_next):
    ...
    if (
        ...
        and not path.startswith("/api/skills/marketplace/install-requests/")
    ):
        if not _has_valid_session_token(request):
            return JSONResponse(
                status_code=401,
...
async def restart_gateway():
    """Kick off a ``notpunks gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``notpunks update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "hermes-update")
    except Exception as exc:
        _log.exception("Failed to spawn notpunks update")
...
# the canonical ``notpunks auth add <provider>`` command so the dashboard
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "notpunks auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "notpunks auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "notpunks auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "notpunks auth add qwen-oauth",
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``notpunks auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
async def install_marketplace_skill_local(body: MarketplaceInstallRequest):
    from hermes_cli.local_bridge import _install_marketplace_skill

    body_data = body.model_dump() if hasattr(body, "model_dump") else body.dict()
    payload, status = _install_marketplace_skill(body_data, origin="dashboard")
    return JSONResponse(status_code=int(status), content=payload)


@app.post("/api/skills/marketplace/wizard-chat")
async def marketplace_skill_wizard_chat_local(request: Request):
    from hermes_cli.local_bridge import _marketplace_skill_wizard_chat
...
@app.post("/api/skills/marketplace/create-draft")
async def create_marketplace_skill_draft_local(request: Request):
    from hermes_cli.local_bridge import _create_marketplace_skill_draft
...
@app.post("/api/skills/marketplace/publish-draft")
async def publish_marketplace_skill_draft_local(request: Request):
    from hermes_cli.local_bridge import _publish_marketplace_skill_draft
...
@app.delete("/api/skills/marketplace/install")
async def uninstall_marketplace_skill_local(body: MarketplaceUninstallRequest):
    from hermes_cli.local_bridge import _uninstall_marketplace_skill

    body_data = body.model_dump() if hasattr(body, "model_dump") else body.dict()
    payload, status = _uninstall_marketplace_skill(body_data)
    return JSONResponse(status_code=int(status), content=payload)


@app.get("/api/skills/marketplace/install-requests/{request_id}")
async def get_marketplace_install_request(request_id: str):
    from hermes_cli.local_bridge import get_install_request
...
@app.post("/api/skills/marketplace/install-requests/{request_id}/{action}")
async def update_marketplace_install_request(request_id: str, action: str):
    from hermes_cli.local_bridge import approve_install_request, deny_install_request
