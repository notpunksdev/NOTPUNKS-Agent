"""notpunks claw — OpenClaw migration commands.

Usage:
    notpunks claw migrate              # Preview then migrate (always shows preview first)
    notpunks claw migrate --dry-run    # Preview only, no changes
    notpunks claw migrate --yes        # Skip confirmation prompt
    notpunks claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    notpunks claw migrate --no-backup  # Skip pre-migration snapshot
    notpunks claw cleanup              # Archive leftover OpenClaw directories
    notpunks claw cleanup --dry-run    # Preview what would be archived
"""
...
def claw_command(args):
    """Route notpunks claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: notpunks claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'notpunks claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: notpunks claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: notpunks claw migrate --yes")
            return
    ...
    # restorable with `notpunks import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: notpunks import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'notpunks claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: notpunks claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: notpunks claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  notpunks claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  notpunks claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  notpunks config set OPENROUTER_API_KEY sk-or-v1-...")
