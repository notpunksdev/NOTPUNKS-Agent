"""
notpunks fallback — manage the fallback provider chain.
...
Subcommands:
  notpunks fallback [list]   Show the current fallback chain (default when no subcommand)
  notpunks fallback add      Pick provider + model via the same picker as `notpunks model`,
                           then append the selection to the chain
  notpunks fallback remove   Pick an entry to delete from the chain
  notpunks fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `notpunks model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  notpunks fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `notpunks model`, then append the selection to the chain."""
    ...
    print("  `notpunks model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `notpunks fallback list` to view, or `notpunks fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``notpunks fallback [subcommand]``."""
