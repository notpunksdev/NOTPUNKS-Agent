"""
Interactive setup wizard for NOTPUNKS Agent.
...
_DOCS_BASE = "https://agent.notpunks.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during notpunks update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("◆ NOTPUNKS Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure NOTPUNKS Agent using environment variables or config commands:")
    print_info("  notpunks config set model.provider custom")
    print_info("  notpunks config set model.base_url http://localhost:8080/v1")
    print_info("  notpunks config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'notpunks setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'notpunks setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'notpunks setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'notpunks setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'notpunks setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'notpunks setup terminal'"))
    ...
    # Spotify (OAuth via notpunks auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'notpunks setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('notpunks setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('notpunks setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('notpunks setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('notpunks setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('notpunks setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('notpunks config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('notpunks config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('notpunks config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('notpunks', Colors.GREEN)}            Start chatting")
    print(f"   {color('notpunks gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('notpunks doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `notpunks tools` and `notpunks setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``notpunks model``)
    ...
    provider added to ``notpunks model`` is automatically available here.
    ...
    # Delegate to the shared notpunks model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: notpunks model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "NOTPUNKS can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one NOTPUNKS can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'notpunks setup' or configure AUXILIARY_VISION_* settings")
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"notpunks setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'notpunks setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where NOTPUNKS runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using NOTPUNKS via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `notpunks setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where NOTPUNKS delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `notpunks update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://agent.notpunks.com/docs/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="NOTPUNKS",
            bot_description="Your NOTPUNKS agent on Slack",
        )
        ...
        print_info(
            "   Re-run `notpunks slack manifest --write` anytime to refresh after "
            "NOTPUNKS adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where NOTPUNKS delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where NOTPUNKS delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'notpunks whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'notpunks whatsapp' to choose your mode (separate bot number")
...
def _setup_bluebubbles():
    ...
    print_info("Connects NOTPUNKS to iMessage via BlueBubbles - a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://agent.notpunks.com/docs/messaging/webhooks/")
    print()
    ...
    print_info("   https://agent.notpunks.com/docs/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  notpunks config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with NOTPUNKS from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'notpunks setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     notpunks config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: notpunks gateway install")
            else:
                print_info("  You can install later: notpunks gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo notpunks gateway install --system")
                print_info("  Or run in foreground:  notpunks gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   notpunks gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   notpunks gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `notpunks setup tools` and `notpunks tools` use the same flow:
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure NOTPUNKS to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point NOTPUNKS at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point NOTPUNKS at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point NOTPUNKS at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point NOTPUNKS at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to NOTPUNKS equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing NOTPUNKS config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in NOTPUNKS.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" != NOTPUNKS yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("NOTPUNKS can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: notpunks claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: notpunks claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in NOTPUNKS (use notpunks claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      notpunks setup           — full or quick (auto-detected)
      notpunks setup model     — just model/provider
      notpunks setup tts       — just text-to-speech
      notpunks setup terminal  — just terminal backend
      notpunks setup gateway   — just messaging platforms
      notpunks setup tools     — just tool configuration
      notpunks setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◆ NOTPUNKS Setup — {label:<32s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ◆ NOTPUNKS Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your NOTPUNKS Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have NOTPUNKS configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'notpunks setup model|terminal|")
        ...
        setup_mode = prompt_choice("How would you like to set up NOTPUNKS?", [
    ...
    print()
    print_info("You can edit these files directly or use 'notpunks config edit'")
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``notpunks chat`` in a fresh process."""
    notpunks_bin = shutil.which("notpunks")
    if notpunks_bin:
        return [notpunks_bin, "chat"]

    legacy_bin = shutil.which("hermes")
    if legacy_bin:
        return [legacy_bin, "chat"]
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch notpunks chat now?", True):
        return

    chat_argv = _resolve_hermes_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch NOTPUNKS automatically. Run 'notpunks chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'notpunks setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    notpunks setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  notpunks setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'notpunks setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect NOTPUNKS to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'notpunks setup gateway'.")
