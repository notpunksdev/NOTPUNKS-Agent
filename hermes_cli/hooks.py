"""notpunks hooks — inspect and manage shell-script hooks.

Usage::

    notpunks hooks list
    notpunks hooks test <event> [--for-tool X] [--payload-file F]
    notpunks hooks revoke <command>
    notpunks hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``notpunks hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: notpunks hooks {list|test|revoke|doctor}")
        print("Run 'notpunks hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `notpunks hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `notpunks hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `notpunks hooks test` and `notpunks hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `notpunks hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `notpunks hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `notpunks hooks doctor`.")
