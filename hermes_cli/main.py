...
Usage:
    ...
    notpunks chat                # Interactive chat
    notpunks gateway             # Run gateway in foreground
    notpunks gateway start       # Start gateway as service
    notpunks gateway stop        # Stop gateway service
    notpunks gateway status      # Show gateway status
    notpunks gateway install     # Install gateway service
    notpunks gateway uninstall   # Uninstall gateway service
    notpunks setup               # Interactive setup wizard
    notpunks logout              # Clear stored authentication
    notpunks status              # Show status of all components
    notpunks cron                # Manage cron jobs
    notpunks cron list           # List cron jobs
    notpunks cron status         # Check if cron scheduler is running
    notpunks doctor              # Check configuration and dependencies
    ...
    notpunks version             Show version
    notpunks update              Update to latest version
    notpunks uninstall           Uninstall NOTPUNKS Agent
    notpunks acp                 Run as an ACP server for editor integration
    notpunks sessions browse     Interactive session picker with search

    notpunks claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to NOTPUNKS_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (notpunks tools, notpunks setup, notpunks model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'notpunks {command_name}' requires an interactive terminal.\n"
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo notpunks {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo notpunks {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  notpunks --tui --resume {target}")
    if title:
        print(f'  notpunks --tui -c "{title}"')
    print()
...
def _launch_tui(
    ...
    if model:
        ...
        env["NOTPUNKS_INFERENCE_MODEL"] = model
    if provider:
        env["HERMES_TUI_PROVIDER"] = provider
        env["HERMES_INFERENCE_PROVIDER"] = provider
        env["NOTPUNKS_INFERENCE_PROVIDER"] = provider
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'notpunks sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like NOTPUNKS isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  notpunks setup")
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'notpunks setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with NOTPUNKS?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: notpunks gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  notpunks gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '◆ NOTPUNKS Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  notpunks gateway")
        ...
        print()
        print("  Or install as a service: notpunks gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'notpunks whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``notpunks model``) and the setup wizard
    ...
    effective_provider = (
        config_provider
        or os.getenv("NOTPUNKS_INFERENCE_PROVIDER")
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'notpunks model' for "
                "available providers, or run 'notpunks doctor' to diagnose config "
                "issues."
            )
...
# `notpunks model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `notpunks model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  NOTPUNKS only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``notpunks model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `notpunks auth` stores device_code tokens),
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `notpunks model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  NOTPUNKS will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates NOTPUNKS turns to `copilot --acp`.")
    print("  NOTPUNKS currently starts its own ACP subprocess for each request.")
    print("  NOTPUNKS uses your selected model as a hint for the Copilot ACP session.")
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   NOTPUNKS typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with NOTPUNKS, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    NOTPUNKS will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'notpunks model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate NOTPUNKS CLI with a provider."""
...
def cmd_slack(args):
    ...
    Dispatches ``notpunks slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: notpunks slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `notpunks slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_backup(args):
    """Back up NOTPUNKS home directory to a zip file."""
...
def cmd_import(args):
    """Restore a NOTPUNKS backup from a zip file."""
...
def cmd_version(args):
    ...
    try:
        ...
        from hermes_cli.config import load_config
        init_skin_from_config(load_config() or {})
        _agent_name = get_active_skin().get_branding("agent_name", "NOTPUNKS Agent")
    except Exception:
        _agent_name = "NOTPUNKS Agent"
    print(f"{_agent_name} v{__version__} ({__release_date__})")
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``notpunks update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``notpunks update`` to stash the
    lockfile — repeatedly.
    """
...
def _warn_stale_dashboard_processes() -> None:
    ...
    ``notpunks dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``notpunks update`` replaces files on disk, the running
    ...
    print("    kill <pid> && notpunks dashboard --port <port> ...")


def _update_via_zip(args):
    """Update NOTPUNKS Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/notpunksdev/NOTPUNKS-Agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="notpunks-update-")
        zip_path = os.path.join(tmp_dir, f"notpunks-agent-{branch}.zip")
    ...
    try:
        ...
        if result.get("stale_removed"):
            print(f"  − {len(result['stale_removed'])} stale bundled skill(s) removed")
        if result.get("stale_user_modified"):
            print(f"  ~ {len(result['stale_user_modified'])} stale user-modified skill(s) kept")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if NOTPUNKS behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but NOTPUNKS couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but NOTPUNKS couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if NOTPUNKS behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `notpunks update`
# =========================================================================

OFFICIAL_REPO_SLUGS = {"notpunksdev/notpunks-agent"}
LEGACY_HERMES_REPO_SLUGS = {"nousresearch/hermes-agent"}
OFFICIAL_REPO_URL = "https://github.com/notpunksdev/NOTPUNKS-Agent.git"
SKIP_UPSTREAM_PROMPT_FILE = ".skip_upstream_prompt"


def _canonical_github_slug(remote_url: str) -> Optional[str]:
    """Return owner/repo for common GitHub HTTPS/SSH remote URL shapes."""
    ...
    if "github.com/" in normalized:
        ...
        # Handles git@github.com:owner/repo and host aliases such as
        # git@github.com-notpunksdev:owner/repo.
...
def _get_remote_url(git_cmd: list[str], cwd: Path, remote: str) -> Optional[str]:
    """Get the URL of a git remote, or None if not set."""
    try:
        result = subprocess.run(
...
def _is_fork(origin_url: Optional[str]) -> bool:
    ...
    if not origin_url:
        return False
    slug = _canonical_github_slug(origin_url)
    if slug:
        return slug not in OFFICIAL_REPO_SLUGS
    return origin_url.rstrip("/").removesuffix(".git") != OFFICIAL_REPO_URL.removesuffix(
        ".git"
    )
...
def _set_upstream_remote(git_cmd: list[str], cwd: Path) -> bool:
    """Point the existing 'upstream' remote at the official NOTPUNKS repo."""
    try:
        result = subprocess.run(
...
def _upstream_points_to_legacy_hermes(git_cmd: list[str], cwd: Path) -> bool:
    upstream_url = _get_remote_url(git_cmd, cwd, "upstream")
    if not upstream_url:
        return False
    slug = _canonical_github_slug(upstream_url)
    return slug in LEGACY_HERMES_REPO_SLUGS
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if has_upstream and _upstream_points_to_legacy_hermes(git_cmd, cwd):
        print()
        print("→ Updating legacy upstream remote to NOTPUNKS Agent...")
        if _set_upstream_remote(git_cmd, cwd):
            print(f"  ✓ Upstream now points to: {OFFICIAL_REPO_URL}")
        else:
            print("  ✗ Failed to update upstream remote. Skipping upstream sync.")
            return
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official NOTPUNKS Agent repository.")
        print("  This means you may miss updates from notpunksdev/NOTPUNKS-Agent.")
        print()
        try:
            response = (
                input("Add official NOTPUNKS repo as 'upstream' remote? [Y/n]: ")
                .strip()
                .lower()
            )
        except (EOFError, KeyboardInterrupt):
...
def _invalidate_update_cache():
    ...
    ``notpunks update``, every profile is now current.
    """
    homes = []
...
def _project_venv_dir() -> Path:
    """Return the virtualenv directory that belongs to this checkout."""
    try:
        current = Path(sys.executable).resolve()
        for parent in current.parents:
            if parent.parent == PROJECT_ROOT and parent.name in {"venv", ".venv"}:
                return parent
    except Exception:
        pass
...
def _repair_command_links() -> None:
    """Point user-facing commands at the active checkout's entrypoint."""
    venv = _project_venv_dir()
    bin_name = "notpunks.exe" if sys.platform == "win32" else "notpunks"
    target = venv / ("Scripts" if sys.platform == "win32" else "bin") / bin_name
    ...
    link_dirs = [Path.home() / ".local" / "bin"]
    try:
        if os.geteuid() == 0:
            link_dirs.insert(0, Path("/usr/local/bin"))
    ...
    for link_dir in link_dirs:
        try:
            ...
            repaired.append(link_dir)
        except OSError as exc:
            print(f"  ⚠ Could not repair command links in {link_dir}: {exc}")
    if repaired:
        dirs = ", ".join(str(path) for path in repaired)
        print(f"  ✓ Repaired command links in {dirs} → {target}")
...
class _UpdateOutputStream:
    """Stream wrapper used during ``notpunks update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``notpunks update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``notpunks update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``notpunks update --gateway``) the update is already
...
def _cmd_update_check():
    """Implement ``notpunks update --check``: fetch and report without installing."""
...
def _auto_update_before_launch() -> None:
    """Install available git updates before starting an interactive agent."""
    if os.environ.get("NOTPUNKS_AUTO_UPDATE_DONE") == "1":
        return
    if os.environ.get("NOTPUNKS_SKIP_AUTO_UPDATE") == "1":
        return

    try:
        from hermes_cli.config import load_config
        updates_cfg = load_config().get("updates", {})
        if isinstance(updates_cfg, dict) and updates_cfg.get("auto_on_launch") is False:
            return
    except Exception:
        pass
    ...
    print(f"◆ NOTPUNKS update available: {behind} commit(s). Installing before launch...")
    env = {
        **os.environ,
        "NOTPUNKS_AUTO_UPDATE_DONE": "1",
        "NOTPUNKS_AUTO_LAUNCH_UPDATE": "1",
    }
    update = subprocess.run(
        [sys.executable, "-m", "hermes_cli.main", "update", "--no-backup"],
        cwd=PROJECT_ROOT,
        env=env,
    )
    if update.returncode != 0:
        print("⚠ Auto-update failed; starting current version.")
        return

    print("✓ Update installed. Restarting NOTPUNKS Agent...")
    os.environ["NOTPUNKS_AUTO_UPDATE_DONE"] = "1"
    os.execv(sys.executable, [sys.executable, "-m", "hermes_cli.main", *sys.argv[1:]])
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``notpunks update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# NOTPUNKS Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``notpunks update``
    ...
    print(f"  Restore:  notpunks import {out_path}")
...
def cmd_update(args):
    """Update NOTPUNKS Agent to the latest version.
    ...
    if is_managed():
        managed_error("update NOTPUNKS Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    auto_launch_update = os.environ.get("NOTPUNKS_AUTO_LAUNCH_UPDATE") == "1"
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl --noproxy '*' -fsSL https://raw.githubusercontent.com/notpunksdev/NOTPUNKS-Agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        prompt_for_restore = (not auto_launch_update) and auto_stash_ref is not None and (
        ...
        if needs_migration:
            ...
            elif auto_launch_update:
                print("  ℹ Auto-update launch — skipping config migration prompt.")
                print("    Run 'notpunks config migrate' later to apply any new config/env options.")
                response = "n"
            elif not (sys.stdin.isatty() and sys.stdout.isatty()):
                print("  ℹ Non-interactive session — skipping config migration prompt.")
                print(
                    "    Run 'notpunks config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: notpunks config migrate")
            else:
                print()
                print("Skipped. Run 'notpunks config migrate' later to configure.")
        else:
        ...
        try:
            _repair_command_links()
            _ensure_fhs_path_guard()
        except Exception as e:
            logger.debug("FHS PATH guard check failed: %s", e)

        # Write exit code *before* the gateway restart attempt.
        # When running as ``notpunks update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: notpunks gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: notpunks -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `notpunks update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy gateway unit(s) detected:")
                ...
                print()
                print("    notpunks gateway migrate-legacy")
                print()
        ...
        print("  notpunks model              # Select provider and model")
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `notpunks profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} -> notpunks -p {p.name}")
                break
        print()
        return
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  notpunks profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     notpunks -p {name} chat")
                else:
...
def cmd_logs(args):
    """View and filter NOTPUNKS log files."""
...
def main():
    """Main entry point for NOTPUNKS CLI."""
    parser = argparse.ArgumentParser(
        prog="notpunks",
        description="NOTPUNKS Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    notpunks                      Start interactive chat
    notpunks chat -q "Hello"        Single query mode
    notpunks -c                   Resume the most recent session
    notpunks -c "my project"      Resume a session by name (latest in lineage)
    notpunks --resume <session_id> Resume a specific session by ID
    notpunks setup                  Run setup wizard
    notpunks logout                 Clear stored authentication
    notpunks auth add <provider>    Add a pooled credential
    notpunks auth list              List pooled credentials
    notpunks auth remove <p> <t>    Remove pooled credential by index, id, or label
    notpunks auth reset <provider>  Clear exhaustion status for a provider
    notpunks model                  Select default model
    notpunks fallback [list]        Show fallback provider chain
    notpunks fallback add           Add a fallback provider (same picker as `notpunks model`)
    notpunks fallback remove        Remove a fallback provider from the chain
    notpunks config                 View configuration
    notpunks config edit            Edit config in $EDITOR
    notpunks config set model gpt-4 Set a config value
    notpunks gateway                Run messaging gateway
    notpunks -s github-auth,deploy-bot
    notpunks -w                   Start in isolated git worktree
    notpunks gateway install        Install gateway background service
    notpunks sessions list          List past sessions
    notpunks sessions browse        Interactive session picker
    notpunks sessions rename ID T   Rename/title a session
    notpunks logs                   View agent.log (last 50 lines)
    notpunks logs -f                Follow agent.log in real time
    notpunks logs errors            View errors.log
    notpunks logs --since 1h        Lines from the last hour
    notpunks debug share             Upload debug report for support
    notpunks update                 Update to latest version

For more help on a command:
    notpunks <command> --help
""",
    )
    ...
    # Mirrors `notpunks chat --model ... --provider ...` semantics.
    parser.add_argument(
        "-m",
        "--model",
        default=None,
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via NOTPUNKS_INFERENCE_MODEL env var."
        ),
    )
    parser.add_argument(
        "--provider",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via NOTPUNKS_INFERENCE_PROVIDER env var."
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to NOTPUNKS_ACCEPT_HOOKS=1 or "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Ignore the NOTPUNKS config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with NOTPUNKS Agent",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also NOTPUNKS_ACCEPT_HOOKS env var and "
    ...
    chat_parser.add_argument(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore the NOTPUNKS config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    chat_parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `notpunks model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy gateway service units from pre-rename installs",
        description=(
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure NOTPUNKS Agent with an interactive wizard. "
        "Run a specific section: notpunks setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'notpunks setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for NOTPUNKS.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "NOTPUNKS")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for NOTPUNKS CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate NOTPUNKS with Spotify via PKCE")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of NOTPUNKS Agent components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in the NOTPUNKS config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist in the NOTPUNKS data directory."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with NOTPUNKS Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your NOTPUNKS setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for NOTPUNKS Agent. Use 'notpunks debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    notpunks debug share              Upload debug report and print URL
    notpunks debug share --lines 500  Include more log lines
    notpunks debug share --expire 30  Keep paste for 30 days
    notpunks debug share --local      Print report locally (no upload)
    notpunks debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'notpunks debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up NOTPUNKS home directory to a zip file",
        description="Create a zip archive of your entire NOTPUNKS configuration, "
        "skills, sessions, and data (excludes the NOTPUNKS Agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/notpunks-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a NOTPUNKS backup from a zip file",
        description="Extract a previously created NOTPUNKS backup into your "
        "NOTPUNKS home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage NOTPUNKS Agent configuration",
    )
    ...
    skills_install_marketplace = skills_subparsers.add_parser(
        "install-marketplace",
        aliases=["install-snft"],
        help="Install a skill from app.notpunks.com marketplace, preferring sNFT cartridges",
    )
    skills_install_marketplace.add_argument("name", help="Marketplace skill name or sNFT metadata URL")
    skills_install_marketplace.add_argument(
        "--category", default="", help="Category folder to install into"
    )
    skills_install_marketplace.add_argument(
        "--force", action="store_true", help="Install despite blocked scan verdict"
    ...
    skills_create = skills_subparsers.add_parser(
        "create", help="Create a starter skill, import SKILL.md, or launch the wizard"
    )
    skills_create.add_argument(
        "name",
        nargs="?",
        default="",
        help="Skill name (lowercase, hyphens/underscores). Omit in terminal CLI to launch the wizard.",
    )
    skills_create.add_argument(
        "--from-file",
        default="",
        help="Path to a SKILL.md file (TUI/slash mode otherwise creates a starter file)",
    )
    skills_create.add_argument(
        "--category", default="", help="Category folder for the skill"
    )
    ...
    skills_delete = skills_subparsers.add_parser(
        "delete", help="Delete a local skill"
    )
    skills_delete.add_argument("name", help="Local skill name to delete")

    skills_reset = skills_subparsers.add_parser(
        "reset",
        help="Reset a bundled skill — clears 'user-modified' tracking so updates work again",
        description=(
            "Clear a bundled skill's entry from the bundled skills sync manifest "
            "so future 'notpunks update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_publish = skills_subparsers.add_parser(
        "publish", help="Publish a skill to the local marketplace or GitHub"
    )
    skills_publish.add_argument("skill_path", help="Skill name or path to skill directory")
    skills_publish.add_argument(
        "--to", default="local", choices=["local", "github", "clawhub"], help="Target registry"
    )
    skills_publish.add_argument(
        "--price", default=0.0, type=float, help="Marketplace price in TON"
    )
    skills_publish.add_argument(
        "--remote", action="store_true", help="Publish to app.notpunks.com marketplace"
    )
    skills_publish.add_argument(
        "--admin-token", action="store_true", help="Use configured server publish token instead of wallet proof"
    )
    skills_publish.add_argument(
        "--repo", default="", help="Target GitHub repo (e.g. openai/skills)"
    )
    skills_unpublish = skills_subparsers.add_parser(
        "unpublish", help="Remove a local or remote marketplace listing"
    )
    skills_unpublish.add_argument("name", help="Marketplace skill name")
    skills_unpublish.add_argument(
        "--remote", action="store_true", help="Remove from app.notpunks.com marketplace"
    )
    skills_unpublish.add_argument(
        "--admin-token", action="store_true", help="Use configured server publish token instead of wallet proof"
    )

    skills_access = skills_subparsers.add_parser(
        "access", help="Show Skill NFT marketplace access"
    )
    skills_access.add_argument("name", nargs="?", default="", help="Optional skill name")
    skills_subparsers.add_parser(
        "marketplace", help="List local Skill NFT marketplace listings"
    )
    skills_marketplace_status = skills_subparsers.add_parser(
        "marketplace-status", help="Show local marketplace install status"
    )
    skills_marketplace_status.add_argument(
        "name", nargs="?", default="", help="Optional marketplace skill name"
    )
    skills_subparsers.add_parser(
        "store", help="Alias for marketplace listings"
    )
    skills_buy = skills_subparsers.add_parser(
        "buy", help="Create a pending Skill NFT purchase intent"
    )
    skills_buy.add_argument("name", help="Marketplace skill name")
    skills_subparsers.add_parser("licenses", help="List owned Skill NFT licenses")
    skills_subparsers.add_parser("earnings", help="Show local creator earnings records")
    ...
    wallet_parser = subparsers.add_parser(
        "wallet",
        help="TON wallet connection and NFT-based skill unlocking",
        description="Connect your TON wallet, scan NFTs, and unlock agent skill modes.",
    )
    wallet_subparsers = wallet_parser.add_subparsers(dest="wallet_action")

    wallet_connect = wallet_subparsers.add_parser("connect", help="Connect a TON wallet via TON Connect")
    wallet_connect.add_argument(
        "--web", action="store_true",
        help="Open a browser page (app.notpunks.com/tonconnect) instead of terminal QR"
    )
    wallet_subparsers.add_parser("status", help="Show wallet and NFT status")
    wallet_subparsers.add_parser("disconnect", help="Disconnect wallet and lock NFT modes")
    wallet_subparsers.add_parser("nft", help="Force NFT re-scan and show unlocked skills")
    wallet_subparsers.add_parser("access", help="Show Skill NFT marketplace access roles")

    def cmd_wallet(args):
        from hermes_cli.wallet_commands import cmd_wallet as _cmd_wallet
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. owner/notpunks-plugin-example)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `notpunks plugins enable <name>`",
    )
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'notpunks tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # notpunks tools list [--platform cli]
    ...
    # notpunks tools disable <name...> [--platform cli]
    ...
    # notpunks tools enable <name...> [--platform cli]
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run NOTPUNKS as an MCP server",
        description=(
            "Manage MCP server connections and run NOTPUNKS as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'notpunks mcp add' to connect to a new server, or\n"
            "'notpunks mcp serve' to expose NOTPUNKS conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run NOTPUNKS as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to NOTPUNKS",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to NOTPUNKS",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of the NOTPUNKS data directory (by default a "
        "single restore-point archive is written to NOTPUNKS backups "
        "before apply; restorable with 'notpunks import').",
    )
    claw_migrate.add_argument(
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update NOTPUNKS Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall NOTPUNKS Agent",
        description="Remove NOTPUNKS Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run NOTPUNKS Agent as an ACP (Agent Client Protocol) server",
        description="Start NOTPUNKS Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch NOTPUNKS Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles - multiple isolated NOTPUNKS instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the NOTPUNKS Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `notpunks --tui` via PTY/WebSocket). "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter NOTPUNKS log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    notpunks logs                    Show last 50 lines of agent.log
    notpunks logs -f                 Follow agent.log in real time
    notpunks logs errors             Show last 50 lines of errors.log
    notpunks logs gateway -n 100     Show last 100 lines of gateway.log
    notpunks logs --level WARNING    Only show WARNING and above
    notpunks logs --session abc123   Filter by session ID
    notpunks logs --component tools  Only show tool-related lines
    notpunks logs --since 1h         Lines from the last hour
    notpunks logs --since 30m -f     Follow, starting from 30 min ago
    notpunks logs list               List available log files with sizes
""",
    )
    ...
    # so introspection/management commands (notpunks hooks list, cron
