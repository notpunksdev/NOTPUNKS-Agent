"""
Unified tool configuration for NOTPUNKS Agent.

`notpunks tools` and `notpunks setup tools` both enter this module.
...
# Platform-scoped toolsets: only appear in the `notpunks tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``notpunks tools`` → "reconfigure existing" would
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        # Run the full `notpunks auth spotify` flow — if the user has no
        ...
        try:
            ...
            _print_info("    Run manually: notpunks auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `notpunks auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: notpunks auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: notpunks plugins enable observability/langfuse")
        _print_info("    Restart Hermes for tracing to take effect.")
        _print_info("    Verify: notpunks plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # otherwise saving via `notpunks tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `notpunks tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `notpunks tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by notpunks tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `notpunks tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `notpunks tools` and `notpunks setup tools`.
