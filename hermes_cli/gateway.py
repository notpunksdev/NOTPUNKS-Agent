...
Handles: notpunks gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `notpunks update`.
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``notpunks update``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}notpunks gateway status\n"
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `notpunks gateway`")
...
SERVICE_DESCRIPTION = "NOTPUNKS Agent Gateway - Messaging Platform Integration"
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    notpunks gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: notpunks gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo notpunks gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    notpunks gateway uninstall")
    print_info("    sudo notpunks gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo notpunks gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo notpunks gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo notpunks gateway start --system")
            return scope, False
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}notpunks gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}notpunks gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `notpunks gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}notpunks gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}notpunks gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}notpunks gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}notpunks gateway start{scope_flag}")
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  notpunks gateway status             # Check status")
...
def launchd_stop():
    ...
    # `notpunks gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: notpunks gateway start")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: notpunks pairing generate bluebubbles",
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'notpunks pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: notpunks pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'notpunks gateway setup'")
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `notpunks gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `notpunks pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `notpunks pairing approve`.")
    elif access_idx == 1:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: notpunks gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  notpunks gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: notpunks gateway run")
                print_info("  For persistence:   tmux new -s hermes 'notpunks gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: notpunks gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup notpunks gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: notpunks gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'notpunks gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: notpunks gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: notpunks gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'notpunks gateway run'")
                print()
            ...
            print()
            print("  notpunks gateway run                              # direct foreground")
            print("  tmux new -s hermes 'notpunks gateway run'         # persistent via tmux")
            print("  nohup notpunks gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: notpunks gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: notpunks gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: notpunks gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: notpunks gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    notpunks gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: notpunks gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  notpunks gateway install")
                    print("  sudo notpunks gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  notpunks gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup notpunks gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'notpunks gateway run'         # persistent via tmux")
                    print("  nohup notpunks gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  notpunks gateway install  # Install as user service")
                    print("  sudo notpunks gateway install --system  # Install as boot-time system service")
