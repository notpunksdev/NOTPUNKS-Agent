...
Toolsets = whatever the user has configured for "cli" in `notpunks tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `notpunks chat`:
...
def run_oneshot(
    ...
    env_model_early = (
        os.getenv("NOTPUNKS_INFERENCE_MODEL", "").strip()
        or os.getenv("HERMES_INFERENCE_MODEL", "").strip()
    )
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "notpunks -z: --provider requires --model (or NOTPUNKS_INFERENCE_MODEL). "
    ...
    os.environ["NOTPUNKS_ACCEPT_HOOKS"] = "1"
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("NOTPUNKS_INFERENCE_PROVIDER", "").strip().lower()
