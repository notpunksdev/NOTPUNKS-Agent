"""``notpunks plugins`` CLI subcommand — install, update, remove, and list plugins.
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]notpunks plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `notpunks plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  notpunks gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] notpunks plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] notpunks plugins")
    console.print("[dim]Enable/disable:[/dim] notpunks plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch notpunks plugins subcommands."""
