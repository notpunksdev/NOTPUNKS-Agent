"""notpunks webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    notpunks webhook subscribe <name> [options]
    notpunks webhook list
    notpunks webhook remove <name>
    notpunks webhook test <name> [--payload '{"key": "value"}']
...
def _setup_hint() -> str:
  ...
  Then start the gateway: notpunks gateway run
"""
...
def webhook_command(args):
    """Entry point for 'notpunks webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: notpunks webhook {subscribe|list|remove|test}")
        print("Run 'notpunks webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (notpunks gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: notpunks webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from notpunks webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (notpunks gateway run)")
