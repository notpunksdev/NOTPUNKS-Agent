...
Shows the status of all NOTPUNKS Agent components.
"""
...
def show_status(args):
    """Show status of all NOTPUNKS Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ◆ NOTPUNKS Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Nous Portal block intentionally omitted — provider disabled in NOTPUNKS

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: notpunks model)'}"
    )
    ...
    # =========================================================================
    # Nous Subscription Features (disabled in NOTPUNKS)
    # =========================================================================
    if False and managed_nous_tools_enabled():
    ...
    # Nous free-tier upgrade-nudge block removed — provider disabled in NOTPUNKS
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: notpunks model)"
    ...
    if not terminal_env:
        ...
        # (notpunks status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   notpunks gateway")
    ...
    print(color("  Run 'notpunks doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'notpunks setup' to configure", Colors.DIM))
    print()
