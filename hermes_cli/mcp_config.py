"""
MCP Server Management CLI — ``notpunks mcp`` subcommand.

Implements ``notpunks mcp add/remove/list/test/configure`` for interactive
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  notpunks mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  notpunks mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  notpunks mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: notpunks mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `notpunks mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  notpunks mcp add <name> --url <endpoint>')
        _info('  notpunks mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `notpunks mcp remove` + `notpunks mcp add` to reconfigure auth.")
        return
...
# ─── notpunks mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'notpunks mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``notpunks mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("notpunks mcp serve                              Run as MCP server")
        _info("notpunks mcp add <name> --url <endpoint>        Add an MCP server")
        _info("notpunks mcp add <name> --command <cmd>         Add a stdio server")
        _info("notpunks mcp add <name> --preset <preset>       Add from a known preset")
        _info("notpunks mcp remove <name>                      Remove a server")
        _info("notpunks mcp list                               List servers")
        _info("notpunks mcp test <name>                        Test connection")
        _info("notpunks mcp configure <name>                   Toggle tools")
        _info("notpunks mcp login <name>                       Re-authenticate OAuth")
        print()
