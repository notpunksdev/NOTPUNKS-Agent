...
TIPS = [
    ...
    "notpunks chat -t web,terminal enables only specific toolsets for a focused session.",
    "notpunks chat -s github-pr-workflow preloads a skill at launch.",
    "notpunks chat -q \"query\" runs a single non-interactive query and exits.",
    "notpunks chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "notpunks chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "notpunks chat --source telegram tags the session for filtering in notpunks sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "notpunks doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "notpunks dump outputs a compact setup summary — great for bug reports.",
    "notpunks config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "notpunks config edit opens config.yaml in your default editor.",
    "notpunks config check scans for missing or stale configuration options.",
    "notpunks sessions browse opens an interactive session picker with search.",
    "notpunks sessions stats shows session counts by platform and database size.",
    "notpunks sessions prune --older-than 30 cleans up old sessions.",
    "notpunks skills search react --source skills-sh searches the skills.sh public directory.",
    "notpunks skills check scans installed hub skills for upstream updates.",
    "notpunks skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "notpunks skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "notpunks mcp add github --command npx adds MCP servers from the command line.",
    "notpunks mcp serve runs Hermes itself as an MCP server for other agents.",
    "notpunks auth add lets you add multiple API keys for credential pool rotation.",
    "notpunks completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "notpunks logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    ...
    "Profile names become shell commands — 'notpunks profile create coder' creates the 'coder' command.",
    "notpunks profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "notpunks sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: notpunks cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "notpunks gateway install sets it up as a system service that starts on boot.",
    ...
    "MCP servers auto-generate toolsets at runtime — notpunks tools can toggle them per platform.",
    ...
    "notpunks chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "notpunks plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "notpunks acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "notpunks login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "notpunks status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'notpunks profile create ls' would be rejected.",
    "notpunks profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "notpunks claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via notpunks tools.",
