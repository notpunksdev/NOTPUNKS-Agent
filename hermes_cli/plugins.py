...
class PluginManifest:
    ...
    # lookups and by ``notpunks plugins list``. For a flat plugin at
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        for manifest in winners.values():
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `notpunks plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``notpunks tools`` TUI so plugin-provided toolsets appear
