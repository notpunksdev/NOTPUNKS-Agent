"""PTY bridge for `notpunks dashboard` chat tab.
...
* **POSIX-only.**  NOTPUNKS Agent supports Windows exclusively via WSL, which
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError(
                    ...
                    "NOTPUNKS Agent supports Windows only via WSL."
                )
