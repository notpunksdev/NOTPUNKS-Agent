...
`notpunks backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the hermes-agent repo and transient files).

`notpunks import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
def run_backup(args) -> None:
    ...
    print(f"\nRestore with: notpunks import {out_path.name}")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: notpunks profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: notpunks update")
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and notpunks backup --quick)
# ---------------------------------------------------------------------------
...
# — `notpunks update` snapshots this set before pulling so approved-user lists
...
def run_quick_backup(args) -> None:
    """CLI entry point for notpunks backup --quick."""
...
def create_pre_update_backup(
    ...
    (``notpunks update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `notpunks claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `notpunks import` and the
