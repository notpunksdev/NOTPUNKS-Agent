"""``notpunks logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    notpunks logs                    # last 50 lines of agent.log
    notpunks logs -f                 # follow agent.log in real time
    notpunks logs errors             # last 50 lines of errors.log
    notpunks logs gateway -n 100    # last 100 lines of gateway.log
    notpunks logs --level WARNING    # only WARNING+ lines
    notpunks logs --session abc123   # filter by session ID substring
    notpunks logs --component tools  # only tool-related lines
    notpunks logs --since 1h         # lines from the last hour
    notpunks logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hermes runs — try 'notpunks chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'notpunks chat' to generate logs)")
