"""
Configuration management for NOTPUNKS Agent.
...
- notpunks config          - Show current configuration
- notpunks config edit     - Open config in editor
- notpunks config set      - Set a specific value
- notpunks config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `notpunks plugins enable
    # observability/langfuse` or `notpunks tools → Langfuse`); credentials gate
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "notpunks update"


def format_managed_message(action: str = "modify this NOTPUNKS installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this NOTPUNKS installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this NOTPUNKS installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this NOTPUNKS installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall NOTPUNKS."
    )
...
DEFAULT_CONFIG = {
    ...
    "providers": {},
    "fallback_providers": [],
    # OpenRouter routing preference for fresh installs. Throughput favors
    # faster token generation for interactive agent sessions.
    "provider_routing": {
        "sort": "throughput",
    },
    ...
    "agent": {
        ...
        "api_max_retries": 3,
        # Global language preference for assistant responses and interactive
        # wizards. Supported: "en", "ru".
        "language": "en",
        "service_tier": "",
        # Keep provider-side reasoning on by default, but at a low effort so
        # fresh installs balance answer quality with interactive latency.
    ...
    # Local bridge for app.notpunks.com marketplace actions.
    "marketplace": {
        "local_bridge": {
            "enabled": True,
            "host": "127.0.0.1",
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *NOTPUNKS Agent*" header.
    ...
    "model_catalog": {
        ...
        # next /model or `notpunks model` invocation; network failures
    ...
    "sessions": {
        ...
        # default of ``notpunks sessions prune``.
        "retention_days": 90,
    ...
    # ``notpunks update`` behaviour.
    "updates": {
        # Check for updates before launching the interactive agent. When a
        # clean git update is available, NOTPUNKS installs it in the
        # background and restarts the launch command once so the new code is
        # active immediately. Set false to keep manual-only updates.
        "auto_on_launch": True,
        # Run a full ``notpunks backup``-style zip of HERMES_HOME before every
        # ``notpunks update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``notpunks import <path>``.  Off by default —
    ...
    # TON Wallet & NFT Access configuration
    "wallet": {
        "enabled": True,
        "address": None,
        "verified": False,
    ...
    "nft": {
        "collections": {
            # ── NOT Punks ♂️ — базовые скиллы ──
            # Collection: EQDP9nGW2Ho0V0_pbW8qpx2q3VJVd9n0BtbQjts2XqZIrfgF
            "not_punks_legal": {
                "address": "EQDP9nGW2Ho0V0_pbW8qpx2q3VJVd9n0BtbQjts2XqZIrfgF",
                "skill_mode": "legal_mode",
                "prompt_modifier": "legal_assistant_ru",
                "description": "NOT Punk → Legal Mode: формальный стиль, анализ рисков, проверка договоров",
            },
            "not_punks_sales": {
                "address": "EQDP9nGW2Ho0V0_pbW8qpx2q3VJVd9n0BtbQjts2XqZIrfgF",
                "skill_mode": "sales_mode",
                "prompt_modifier": "sales_assistant",
                "description": "NOT Punk → Sales Mode: sales copy, outreach, CRM automation",
            },
            "not_punks_creative": {
                "address": "EQDP9nGW2Ho0V0_pbW8qpx2q3VJVd9n0BtbQjts2XqZIrfgF",
                "skill_mode": "creative_mode",
                "prompt_modifier": "creative_assistant",
                "description": "NOT Punk → Creative Mode: дизайн, генерация идей, контент-креатив",
            },
            "not_punks_research": {
                "address": "EQDP9nGW2Ho0V0_pbW8qpx2q3VJVd9n0BtbQjts2XqZIrfgF",
                "skill_mode": "research_mode",
                "prompt_modifier": "research_assistant",
                "description": "NOT Punk → Research Mode: аналитика, исследования, data-driven insights",
            },

            # ── NOT Punks Girls ♀️ — базовые скиллы (такие же) ──
            # Collection: EQDJAXZJDQkBP6vCGn27vW00GIlUk_AQXnVtqNV0Tq2nFUyB
            "not_punks_girls_legal": {
                "address": "EQDJAXZJDQkBP6vCGn27vW00GIlUk_AQXnVtqNV0Tq2nFUyB",
                "skill_mode": "legal_mode",
                "prompt_modifier": "legal_assistant_ru",
                "description": "NOT Punk Girl → Legal Mode: формальный стиль, анализ рисков, проверка договоров",
            },
            "not_punks_girls_sales": {
                "address": "EQDJAXZJDQkBP6vCGn27vW00GIlUk_AQXnVtqNV0Tq2nFUyB",
                "skill_mode": "sales_mode",
                "prompt_modifier": "sales_assistant",
                "description": "NOT Punk Girl → Sales Mode: sales copy, outreach, CRM automation",
            },
            "not_punks_girls_creative": {
                "address": "EQDJAXZJDQkBP6vCGn27vW00GIlUk_AQXnVtqNV0Tq2nFUyB",
                "skill_mode": "creative_mode",
                "prompt_modifier": "creative_assistant",
                "description": "NOT Punk Girl → Creative Mode: дизайн, генерация идей, контент-креатив",
            },
            "not_punks_girls_research": {
                "address": "EQDJAXZJDQkBP6vCGn27vW00GIlUk_AQXnVtqNV0Tq2nFUyB",
                "skill_mode": "research_mode",
                "prompt_modifier": "research_assistant",
                "description": "NOT Punk Girl → Research Mode: аналитика, исследования, data-driven insights",
            },

            # ── TNO Elemental Kids — премиальные скиллы (только с premium: true) ──
            # Collection: EQCGbQyAJxxMsYQWLCklkXQq4fkIBK3kz3GA1TkFJyUR9nTH
            "earth_kid": {
                "address": "EQCGbQyAJxxMsYQWLCklkXQq4fkIBK3kz3GA1TkFJyUR9nTH",
                "element_filter": "element earth",
                "skill_mode": "premium_legal_mode",
                "prompt_modifier": "legal_assistant_ru",
                "description": "Earth Kid → Premium Legal Mode: элитный юридический анализ, стратегическое планирование",
                "premium": True,
            },
            "fire_kid": {
                "address": "EQCGbQyAJxxMsYQWLCklkXQq4fkIBK3kz3GA1TkFJyUR9nTH",
                "element_filter": "element fire",
                "skill_mode": "premium_sales_mode",
                "prompt_modifier": "sales_assistant",
                "description": "Fire Kid → Premium Sales Mode: advanced sales strategy, high-ticket closing",
                "premium": True,
            },
            "water_kid": {
                "address": "EQCGbQyAJxxMsYQWLCklkXQq4fkIBK3kz3GA1TkFJyUR9nTH",
                "element_filter": "element water",
                "skill_mode": "premium_creative_mode",
                "prompt_modifier": "creative_assistant",
                "description": "Water Kid → Premium Creative Mode: premium design, brand strategy, creative direction",
                "premium": True,
            },
            "air_kid": {
                "address": "EQCGbQyAJxxMsYQWLCklkXQq4fkIBK3kz3GA1TkFJyUR9nTH",
                "element_filter": "element air",
                "skill_mode": "premium_research_mode",
                "prompt_modifier": "research_assistant",
                "description": "Air Kid → Premium Research Mode: deep analytics, competitive intelligence, forecasting",
                "premium": True,
            },
            "notpunks_skill_nft": {
                "address": "EQDwJtSrCsItP-B-XmhQkdnIrp-OTYluTeJNYGseyq-eR2Gd",
                "skill_mode": "skill_license_mode",
                "description": "NOTPUNKS Skill NFT → transferable marketplace skill license",
            },
        },
    },
...
OPTIONAL_ENV_VARS = {
    "TONCENTER_API_KEY": {
        "description": "TON Center API key for wallet NFT scans",
        "prompt": "TON Center API key",
    ...
    "TONAPI_API_KEY": {
        "description": "TonAPI key for faster wallet NFT scans",
        "prompt": "TonAPI key",
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'notpunks model' for endpoint-specific config)",
    ...
    "NOTPUNKS_SNFT_CHAIN": {
        "description": "Preferred sNFT unlock adapter chain for marketplace installs (ton, evm, solana)",
        "prompt": "Preferred sNFT chain",
        "url": None,
        "password": False,
        "category": "setting",
        "advanced": True,
    },
    "NOTPUNKS_SNFT_UNLOCK_REQUEST": {
        "description": "Pre-signed normalized sNFT unlock request JSON for EVM/Solana/manual adapters",
        "prompt": "sNFT unlock request JSON",
        "url": None,
        "password": True,
        "category": "setting",
        "advanced": True,
    ...
    "NOTPUNKS_SNFT_UNLOCK_REQUEST_FILE": {
        "description": "Path to a pre-signed normalized sNFT unlock request JSON file",
        "prompt": "sNFT unlock request file",
        "url": None,
        "password": False,
        "category": "setting",
        "advanced": True,
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'notpunks setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — NOTPUNKS won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'notpunks doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `notpunks plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: notpunks config set <key> <value>")
...
#   openai-codex (OAuth — notpunks auth) — OpenAI Codex
#   nous         (OAuth — notpunks auth) — Nous Portal
...
def show_config():
    ...
    print(color("│              ◆ NOTPUNKS Configuration                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(f"  Language:     {config.get('agent', {}).get('language', DEFAULT_CONFIG['agent']['language'])}")
    ...
    print(color("  notpunks config edit     # Edit config file", Colors.DIM))
    print(color("  notpunks config set <key> <value>", Colors.DIM))
    print(color("  notpunks setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: notpunks config set <key> <value>")
            print()
            print("Examples:")
            print("  notpunks config set model anthropic/claude-sonnet-4")
            print("  notpunks config set terminal.backend docker")
            print("  notpunks config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'notpunks config migrate' to add them")
        ...
        print("  notpunks config           Show current configuration")
        print("  notpunks config edit      Open config in editor")
        print("  notpunks config set <key> <value>   Set a config value")
        print("  notpunks config check     Check for missing/outdated config")
        print("  notpunks config migrate   Update config with new options")
        print("  notpunks config path      Show config file path")
        print("  notpunks config env-path  Show .env file path")
        sys.exit(1)
