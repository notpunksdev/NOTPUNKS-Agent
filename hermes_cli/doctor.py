...
Diagnoses issues with NOTPUNKS Agent setup.
"""
...
def run_doctor(args):
    ...
    _doctor_label = f"🩺 {_agent_name} Doctor"
    _doctor_line = "│" + _doctor_label.center(57) + "│"
    print(color(_doctor_line, Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'notpunks setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'notpunks setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'notpunks setup' to create one")
                issues.append("Run 'notpunks setup' to create .env")
            ...
            if provider and provider != "auto":
                if canonical_provider is None or (known_providers and canonical_provider not in known_providers):
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'notpunks config set model.provider <valid_provider>'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.hermes/.env or run 'notpunks setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{canonical_provider}'. "
                                f"Run 'notpunks setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'notpunks config set model.provider <name>'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'notpunks setup' to migrate config")
                else:
                    issues.append("Run 'notpunks doctor --fix' or 'notpunks setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'notpunks doctor --fix'")
        except Exception:
            pass
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give NOTPUNKS a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# NOTPUNKS Agent Persona\n\n"
                "<!-- Edit this file to customize how NOTPUNKS communicates. -->\n\n"
                "You are NOTPUNKS, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'notpunks doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            for _exe_name in ("notpunks", "hermes"):
                _candidate = PROJECT_ROOT / _venv_name / "bin" / _exe_name
        ...
        _cmd_link = _cmd_link_dir / "notpunks"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(notpunks not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/notpunks → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/notpunks points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/notpunks → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/notpunks — run 'notpunks doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/notpunks exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/notpunks not found",
                    "(notpunks command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/notpunks → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/notpunks symlink — run 'notpunks doctor --fix'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'notpunks config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'notpunks setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: notpunks skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: notpunks memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: notpunks memory setup")
                issues.append("No Honcho API key — run 'notpunks memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run notpunks memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: notpunks memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: notpunks memory setup")
        except Exception as _e:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'notpunks doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
