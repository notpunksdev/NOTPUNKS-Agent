"""ACP agent server — exposes NOTPUNKS Agent via the Agent Client Protocol."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping NOTPUNKS AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show NOTPUNKS version",
        },
    )
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate NOTPUNKS using the currently configured {provider} runtime credentials.",
                )
            ]
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # NOTPUNKS' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run NOTPUNKS on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"NOTPUNKS Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when NOTPUNKS has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
