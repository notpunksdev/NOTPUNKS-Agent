...
DEFAULT_AGENT_IDENTITY = (
    "You are NOTPUNKS Agent, an intelligent AI assistant created by NOTPUNKS. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using NOTPUNKS Agent "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot NOTPUNKS Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hermes-agent` skill "
            "first. It has the actual commands (e.g. `notpunks config set …`, `notpunks tools`, "
            "`notpunks setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: notpunks setup, notpunks setup tools, notpunks setup terminal, notpunks status.",
        ]
    )
    return "\n".join(lines)
...
# =============================================================================
# Wallet / NFT Context Prompt Builder
# =============================================================================

def build_wallet_context_prompt(wallet_ctx) -> str:
    ...
    lines.append(f"- Connected wallet: `{wallet_ctx.wallet_address}`")
    lines.append(f"- Network: {wallet_ctx.network}")
    lines.append(f"- Verified: {'yes' if wallet_ctx.verified else 'no'}")

    if wallet_ctx.active_modes:
        lines.append("")
        lines.append("### Unlocked Agent Modes")
        for mode in wallet_ctx.active_modes:
        ...
        mapper = SkillMapper()
        modifier = mapper.get_prompt_modifier(wallet_ctx.active_modes)
        if modifier:
            lines.append("")
