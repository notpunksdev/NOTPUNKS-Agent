...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. NOTPUNKS also honours HERMES_CA_BUNDLE (its own convention)
...
def _requests_get(url: str, *, local_endpoint: bool = False, **kwargs):
    """requests.get wrapper that bypasses proxies for local model servers."""
    if local_endpoint:
        kwargs.setdefault("proxies", {"http": None, "https": None})
    return requests.get(url, **kwargs)


def _httpx_client(*, local_endpoint: bool = False, **kwargs):
    """httpx.Client wrapper that bypasses proxies for local model servers."""
    if local_endpoint:
...
# Minimum context length required to run NOTPUNKS Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent NOTPUNKS from probing-down to
...
def detect_local_server_type(base_url: str, api_key: str = "") -> Optional[str]:
    ...
    try:
        with _httpx_client(timeout=2.0, headers=headers, local_endpoint=local_endpoint) as client:
...
def fetch_endpoint_model_metadata(
    ...
    local_endpoint = is_local_endpoint(normalized)
        ...
        try:
            ...
            if is_llamacpp:
                try:
                    ...
                    props_resp = _requests_get(base + "/v1/props", headers=headers, timeout=5, verify=_verify, local_endpoint=local_endpoint)
                    if not props_resp.ok:
                        props_resp = _requests_get(base + "/props", headers=headers, timeout=5, verify=_verify, local_endpoint=local_endpoint)
                    if props_resp.ok:
...
def query_ollama_num_ctx(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    ...
    try:
        with _httpx_client(timeout=3.0, headers=headers, local_endpoint=is_local_endpoint(base_url)) as client:
        ...
        with _httpx_client(timeout=3.0, headers=headers, local_endpoint=is_local_endpoint(base_url)) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # NOTPUNKS grow conversations past the runtime limit and Ollama
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets NOTPUNKS sends to providers:
