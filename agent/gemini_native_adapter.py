...
NOTPUNKS keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for NOTPUNKS's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with NOTPUNKS)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). NOTPUNKS typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with NOTPUNKS retry/error classification."""
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "(get one at https://aistudio.google.com/app/apikey), or run `notpunks setup` "
