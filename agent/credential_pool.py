...
class CredentialPool:
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other NOTPUNKS profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `notpunks auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, NOTPUNKS PKCE)
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `notpunks auth add nous --label <name>`).  Fall back to the
        ...
        # NOTPUNKS auth store or env vars, so resolve them here.
        ...
        # Respect user suppression — `notpunks auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the NOTPUNKS auth store.  Without this gate the removal is instantly
        ...
        # NOTPUNKS owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `notpunks auth openai-codex`.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    changed = False
    active_sources: Set[str] = set()
    # Honour user suppression — `notpunks auth remove <provider> <N>` for an
