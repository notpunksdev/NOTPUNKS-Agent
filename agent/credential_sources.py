"""Unified removal contract for every credential source NOTPUNKS reads from.

NOTPUNKS seeds its credential pool from many places:
    ...
    manual        — user ran `notpunks auth add`
...
unify here is **removal**:
    ...
    ``notpunks auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to NOTPUNKS.",
            f"  The pool entry is now suppressed — NOTPUNKS will ignore "
            f"{env_var} until you run `notpunks auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so NOTPUNKS stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `notpunks auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared NOTPUNKS Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `notpunks login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `notpunks auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the NOTPUNKS auth store is not enough — _seed_from_singletons() would
    ...
    via ``notpunks auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `notpunks auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `notpunks auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    NOTPUNKS stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `notpunks auth add copilot` to re-enable if needed.",
    ])
