...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time NOTPUNKS starts and finds ``~/.openclaw/``.
    ...
    get carried forward and the agent dutifully reads them). ``notpunks claw
    ...
    return (
        ...
        "directory's config/memory instead of NOTPUNKS's.\n"
        "Run `notpunks claw cleanup` to archive it (rename → .openclaw.pre-migration). "
        "This tip only shows once; rerun it any time with `notpunks claw cleanup`."
    )
