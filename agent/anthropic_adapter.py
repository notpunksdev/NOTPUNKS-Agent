"""Anthropic Messages API adapter for NOTPUNKS Agent.

Translates between NOTPUNKS's internal OpenAI-style message format and
...
# NOTPUNKS effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    NOTPUNKS historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by NOTPUNKS)
    ...
    # 1. NOTPUNKS-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration NOTPUNKS configs.
...
# ── NOTPUNKS-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run NOTPUNKS-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize NOTPUNKS with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read NOTPUNKS-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read NOTPUNKS OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("NOTPUNKS Agent", "Claude Code")
                text = text.replace("NOTPUNKS agent", "Claude Code")
                text = text.replace("hermes-agent", "claude-code")
                text = text.replace("NOTPUNKS", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that NOTPUNKS surfaces in its CLI. We
