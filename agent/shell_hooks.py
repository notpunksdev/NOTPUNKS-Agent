...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # NOTPUNKS-canonical
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``notpunks hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a NOTPUNKS wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical NOTPUNKS
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ NOTPUNKS is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``NOTPUNKS_ACCEPT_HOOKS`` env var (or legacy ``HERMES_ACCEPT_HOOKS``)
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = (
        os.environ.get("NOTPUNKS_ACCEPT_HOOKS", "").strip().lower()
        or os.environ.get("HERMES_ACCEPT_HOOKS", "").strip().lower()
    )
...
# ---------------------------------------------------------------------------
# Introspection (used by `notpunks hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``notpunks hooks test`` and ``notpunks hooks doctor``.
    ...
    produce — otherwise scripts tested via ``notpunks hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical NOTPUNKS-wire-shape response."""
