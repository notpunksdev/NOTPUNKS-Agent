# Contributing to NOTPUNKS Agent

Thank you for contributing to NOTPUNKS Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want NOTPUNKS to work everywhere.
...
Bundled skills (in `skills/`) ship with every NOTPUNKS install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [NOTPUNKS Discord](https://discord.gg/NousResearch). Users can install it with `hermes skills install`.
...
The user may skip setup and keep loading the skill. NOTPUNKS only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing NOTPUNKS tools (`web_extract`, `terminal`, `read_file`).
...
NOTPUNKS uses a data-driven skin system — no code changes needed to add a new skin.
...
NOTPUNKS runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
NOTPUNKS has terminal access. Security matters.
...
- Include: OS, Python version, NOTPUNKS version (`hermes version`), full error traceback
