...
def cmd_sync(args) -> None:
    ...
    Scans all NOTPUNKS profiles and creates host blocks for any that don't
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `notpunks update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current NOTPUNKS profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives NOTPUNKS persistent cross-session memory.")
    ...
    try:
        ...
        print("  Run: notpunks config set memory.provider honcho")
...
def _active_profile_name() -> str:
    """Return the active NOTPUNKS profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    NOTPUNKS' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. NOTPUNKS asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → NOTPUNKS + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → NOTPUNKS\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives NOTPUNKS persistent memory")
    ...
    if user_files:
        ...
        print("  (NOTPUNKS calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In NOTPUNKS, they are seeded once into Honcho's AI peer through the")
    ...
    print("    NOTPUNKS:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    NOTPUNKS:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    NOTPUNKS:   Honcho observes every message and updates representations")
    ...
    print("    NOTPUNKS:   per-session by default — each run gets its own session")
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'notpunks memory setup'...\n")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to notpunks memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to NOTPUNKS Honcho",
    )
