"""langfuse — NOTPUNKS plugin for Langfuse observability.

Traces NOTPUNKS conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the NOTPUNKS plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``notpunks plugins enable
observability/langfuse`` or ``notpunks tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``notpunks tools`` or ~/.hermes/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the NOTPUNKS plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older NOTPUNKS branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current NOTPUNKS also has a turn-scoped
    ...
    # messages is a list only for legacy NOTPUNKS branches that fired
    # pre_llm_call with API messages directly. Current NOTPUNKS fires
...
def register(ctx) -> None:
    ...
    # NOTPUNKS versions.  pre_api_request / post_api_request fire per API
