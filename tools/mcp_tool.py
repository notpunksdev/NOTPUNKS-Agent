...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `notpunks mcp login {server_name}` (or delete the tokens "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the NOTPUNKS config file.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves NOTPUNKS's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the NOTPUNKS registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``notpunks tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the NOTPUNKS registry.
