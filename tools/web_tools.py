...
Backend is selected during ``notpunks tools`` setup (web.backend in config.yaml).
When available, NOTPUNKS can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``notpunks tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "run `notpunks tools` and select Nous Subscription as the web provider."
        )
