...
Update logic:
  ...
  - REMOVED from bundled (in manifest, gone from repo): cleaned from manifest
    and removed from the user skills dir if the local copy is still unmodified.
...
def _find_local_skill_dirs_by_name(skill_name: str) -> List[Path]:
    """Return local skill directories whose frontmatter name matches skill_name."""
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if not bundled_dir.exists():
        return {
            ...
            "stale_removed": [], "stale_user_modified": [],
            "total_bundled": 0,
        }
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `notpunks skills reset {skill_name}` "
    ...
    # Clean stale manifest entries (skills removed from bundled dir). If the
    # user's local copy is byte-identical to the last bundled origin hash, remove
    # it too: this lets the distribution shrink the default skill set without
    # leaving old upstream skills visible forever. User-modified copies are kept.
    ...
    for name in cleaned:
        ...
        if origin_hash:
            for local_dir in _find_local_skill_dirs_by_name(name):
                if not local_dir.exists():
                    continue
                if _dir_hash(local_dir) == origin_hash:
                    try:
                        shutil.rmtree(local_dir)
                        stale_removed.append(name)
                    except (OSError, IOError) as e:
                        logger.debug("Could not remove stale bundled skill %s at %s: %s", name, local_dir, e)
                else:
                    stale_user_modified.append(name)
        del manifest[name]
    ...
    return {
        ...
        "cleaned": cleaned,
        "stale_removed": sorted(set(stale_removed)),
        "stale_user_modified": sorted(set(stale_user_modified)),
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `notpunks skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `notpunks update` runs "
...
if __name__ == "__main__":
    ...
    if result.get("stale_removed"):
        parts.append(f"{len(result['stale_removed'])} stale bundled removed")
