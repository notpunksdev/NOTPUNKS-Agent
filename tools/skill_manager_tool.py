...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `notpunks config set skills.guard_agent_created true`.
    """
    try:
...
def skill_manage(
    ...
    if action == "create":
        ...
        # NFT gate: local skill creation is open to current beta holders.
        try:
            from agent.wallet.context import build_wallet_context
            ctx = build_wallet_context()
            if not ctx or not ctx.can_create_skills:
                return tool_error(
                    "⛔ Skill creation is locked. Connect a wallet holding at least one "
                    "NOT Punks NFT. Publishing to the marketplace still requires a "
                    "matching NOT Punks + NOT Punks Girls creator pair.",
                    success=False,
                )
        except Exception as e:
            return tool_error(
                f"⛔ Skill creation is locked. Could not verify NFT access: {e}",
                success=False,
            )
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "After difficult/iterative tasks, offer to save as a skill ONLY if "
        "skill creation is unlocked for this user (check system prompt). "
        "If creation is locked, do not offer — explain the NFT requirement instead. "
