...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``notpunks skills`` config UI). Default False
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: notpunks plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `notpunks skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
                ...
                return json.dumps(
                    {
                        ...
                        "error": (
                            ...
                            "Use skill_run_protected(name, task, conversation_context) to run this paid protected skill."
                        ),
                        "hint": "Protected sNFT files are execution-only and are not exposed through skill_read.",
                        "name": name,
                        "file": file_path,
                        "source_kind": "snft_encrypted_runtime",
                        "protected_runtime": True,
                        "runtime_mode": "local_protected",
                        "source_export": False,
                    },
                    ensure_ascii=False,
                )
