...
selectable via ``notpunks tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `notpunks tools` → Image Generation."
            ) from exc
        raise
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `notpunks plugins list` to see "
