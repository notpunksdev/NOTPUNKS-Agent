#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the NOTPUNKS Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by NOTPUNKS) but not activated
...
# ---------------------------------------------------------------------------
# NOTPUNKS centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("NOTPUNKS index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("NOTPUNKS index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized NOTPUNKS Skills Index.
