...
In single-bot mode (what NOTPUNKS currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local NOTPUNKS gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that NOTPUNKS does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; NOTPUNKSAgent/1.0)",
                    "Accept": "*/*",
                },
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by NOTPUNKS webhook mode")
    ...
    async def _resolve_sender_profile(self, sender_id: Any) -> Dict[str, Optional[str]]:
        """Map Feishu's three-tier user IDs onto NOTPUNKS' SessionSource fields.
    ...
    def _is_self_sent_bot_message(self, event: Any) -> bool:
        """Return True only for Feishu events emitted by this NOTPUNKS bot."""
        sender = getattr(event, "sender", None)
...
# Called by `notpunks gateway setup` via _setup_feishu() in hermes_cli/gateway.py.
# =============================================================================
