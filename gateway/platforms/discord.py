...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields NOTPUNKS manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your NOTPUNKS session")
        ...
        @tree.command(name="status", description="Show NOTPUNKS session status")
        ...
        @tree.command(name="stop", description="Stop the running NOTPUNKS agent")
        ...
        @tree.command(name="update", description="Update NOTPUNKS Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the NOTPUNKS gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a NOTPUNKS session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to NOTPUNKS in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a NOTPUNKS skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new NOTPUNKS session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by NOTPUNKS: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "NOTPUNKS"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by NOTPUNKS: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``notpunks update --gateway``
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``notpunks update`` prompts.
